...
/// \todo remove code repetition, see `base/template_util.h`
...
#include <basic/core/customization_point.h>
...
struct invoke_fn {
  ...
  BASIC_ERASE constexpr auto operator()(F&& f, A&&... a) const
  ...
  BASIC_ERASE constexpr auto operator()(M C::*f, A&&... a) const
...
BASIC_INLINE_VARIABLE constexpr invoke_fn invoke;
...
struct traits {
  ...
  using result = decltype( //
      BASIC_DECLVAL(F &&)(BASIC_DECLVAL(A &&)...));
  template <typename... A>
...
struct traits_member_ptr {
  template <typename... A>
  using result = decltype( //
      std::mem_fn(BASIC_DECLVAL(P))(BASIC_DECLVAL(A &&)...));
  template <typename... A>
  static constexpr bool nothrow = noexcept( //
      std::mem_fn(BASIC_DECLVAL(P))(BASIC_DECLVAL(A&&)...));
};
...
template <typename Void, typename F, typename... A>
BASIC_INLINE_VARIABLE constexpr bool is_invocable_v = false;

template <typename F, typename... A>
BASIC_INLINE_VARIABLE constexpr bool
    is_invocable_v<void_t<invoke_result_t<F, A...>>, F, A...> = true;

template <typename Void, typename R, typename F, typename... A>
BASIC_INLINE_VARIABLE constexpr bool is_invocable_r_v = false;
...
template <typename Void, typename F, typename... A>
BASIC_INLINE_VARIABLE constexpr bool is_nothrow_invocable_v = false;
...
template <typename Void, typename R, typename F, typename... A>
BASIC_INLINE_VARIABLE constexpr bool is_nothrow_invocable_r_v = false;
...
template <typename F, typename... A>
BASIC_INLINE_VARIABLE constexpr bool is_invocable_v =
...
template <typename R, typename F, typename... A>
BASIC_INLINE_VARIABLE constexpr bool is_invocable_r_v =
...
template <typename F, typename... A>
BASIC_INLINE_VARIABLE constexpr bool is_nothrow_invocable_v =
...
BASIC_INLINE_VARIABLE constexpr bool is_nothrow_invocable_r_v =
...
struct invoke_traits_base_<true, I> {
  BASIC_INLINE_VARIABLE static constexpr I invoke{};
};
template <typename I>
...
#define BASIC_DETAIL_CREATE_FREE_INVOKE_TRAITS_USING_1(_, funcname, ns) \
  using ns::funcname;

#define BASIC_DETAIL_CREATE_FREE_INVOKE_TRAITS_USING(_, funcname, ...) \
  BOOST_PP_EXPR_IIF(                                                   \
      BOOST_PP_NOT(BOOST_PP_IS_EMPTY(__VA_ARGS__)),                    \
      BOOST_PP_LIST_FOR_EACH(                                          \
          BASIC_DETAIL_CREATE_FREE_INVOKE_TRAITS_USING_1,              \
          funcname,                                                    \
          BOOST_PP_TUPLE_TO_LIST((__VA_ARGS__))))

/***
 *  BASIC_CREATE_FREE_INVOKER
 *
 *  Used to create an invoker type bound to a specific free-invocable name.
 *
 *  Example:
 *
 *    BASIC_CREATE_FREE_INVOKER(foo_invoker, foo);
 *
 ...
 *    }
 *
 *    using traits = basic::invoke_traits<foo_invoker>;
 *
 ...
 *
 *    BASIC_CREATE_FREE_INVOKER(swap_invoker, swap, std);
 *
 ...
 */
#define BASIC_CREATE_FREE_INVOKER(classname, funcname, ...)                \
  namespace classname##__basic_detail_invoke_ns {                          \
    MAYBE_UNUSED void funcname(                                      \
        ::basic::detail::invoke_private_overload&);                        \
    BASIC_DETAIL_CREATE_FREE_INVOKE_TRAITS_USING(_, funcname, __VA_ARGS__) \
    struct __basic_detail_invoke_obj {                                     \
      template <typename... Args>                                          \
      MAYBE_UNUSED BASIC_ERASE_HACK_GCC constexpr auto operator()(   \
          Args&&... args) const                                            \
  ...
  struct classname                                                         \
      : classname##__basic_detail_invoke_ns::__basic_detail_invoke_obj {}

/***
 *  BASIC_CREATE_MEMBER_INVOKER
 *
 *  Used to create an invoker type bound to a specific member-invocable name.
 *
 *  Example:
 *
 *    BASIC_CREATE_MEMBER_INVOKER(foo_invoker, foo);
 *
 *  The type `foo_invoker` is generated in the current namespace and may be used
 *  as follows:
 *
 ...
 *    traits::is_nothrow_invocable_v<char*, CanFoo, Car&&> // false
 */
#define BASIC_CREATE_MEMBER_INVOKER(classname, membername)                 \
  struct classname {                                                       \
...
/***
 *  BASIC_CREATE_STATIC_MEMBER_INVOKER
 *
 ...
 *  Example:
 *
 *    BASIC_CREATE_STATIC_MEMBER_INVOKER(foo_invoker, foo);
 *
 ...
 *    };
 *
 *    using traits = basic::invoke_traits<foo_invoker<CanFoo>>;
 *
 *    traits::invoke(Car{}) // 2
 *
 ...
 */
#define BASIC_CREATE_STATIC_MEMBER_INVOKER(classname, membername)             \
  template <typename T>                                                       \
  struct classname {                                                          \
    template <typename... Args, typename U = T>                               \
    MAYBE_UNUSED BASIC_ERASE constexpr auto operator()(Args&&... args)  \
...
auto try_tag_invoke(int) noexcept(
    noexcept(tag_invoke(BASIC_DECLVAL(Tag&&), BASIC_DECLVAL(Args&&)...)))
    -> decltype(
        static_cast<void>(
            tag_invoke(BASIC_DECLVAL(Tag &&), BASIC_DECLVAL(Args&&)...)),
        std::true_type{});
...
// The expression basic::tag_invoke(tag, args...) is equivalent to performing
...
//         noexcept(basic::is_nothrow_tag_invocable_v<FooCpo, A, B>)
//         -> basic::tag_invoke_result_t<FooCpo, A, B> {
//       return basic::tag_invoke(*this, (A&&)a, (B&&)b);
//     }
//   };
//   BASIC_DEFINE_CPO(FooCpo, Foo)
//
// And then customising the Foo CPO for a particular type:
//   class SomeType {
//     ...
//     template<typename B>
//     friend int tag_invoke(basic::cpo_t<Foo>, const SomeType& a, B&& b) {
//       // implementation goes here
//     }
//   };
//
// For more details see the C++ standards proposal: https://wg21.link/P1895R0.
BASIC_DEFINE_CPO(detail_tag_invoke_fn::tag_invoke_fn, tag_invoke)

// Query if the 'basic::tag_invoke()' CPO can be invoked with a tag and
...
template <typename Tag, typename... Args>
BASIC_INLINE_VARIABLE constexpr bool is_tag_invocable_v =
...
// Query whether the 'basic::tag_invoke()' CPO can be invoked with a tag
// and arguments of the specified type and that such an invocation is
// noexcept.

template <typename Tag, typename... Args>
BASIC_INLINE_VARIABLE constexpr bool is_nothrow_tag_invocable_v =
...
using is_tag_invocable_r =
    basic::is_invocable_r<R, decltype(basic::tag_invoke), Tag, Args...>;

template <typename R, typename Tag, typename... Args>
BASIC_INLINE_VARIABLE constexpr bool is_tag_invocable_r_v =
    is_tag_invocable_r<R, decltype(basic::tag_invoke), Tag, Args...>::value;

template <typename R, typename Tag, typename... Args>
using is_nothrow_tag_invocable_r =
    basic::is_nothrow_invocable_r<R, decltype(basic::tag_invoke), Tag, Args...>;

template <typename R, typename Tag, typename... Args>
BASIC_INLINE_VARIABLE constexpr bool is_nothrow_tag_invocable_r_v =
