...
#include "basic/varint/fast_varint.h" // IWYU pragma: associated
...
basic::Status NotEnoughEncodedBytes(size_t decoded_varint_size, size_t bytes_provided) {
...
using DecodedSignedPair = std::pair<int64_t, size_t>;

basic::StatusOr<DecodedSignedPair> FastDecodeSignedVarInt(const uint8_t* src, size_t src_size) {
  typedef DecodedSignedPair ResultType;
  RETURN_ERROR_IF(src_size == 0)
    << "Cannot decode a variable-length integer of zero size";
...
basic::Status FastDecodeSignedVarInt(
...
basic::Status FastDecodeSignedVarInt(const std::string& encoded, int64_t* v, size_t* decoded_size) {
  return FastDecodeSignedVarInt(
      fast_varint_detail::to_uchar_ptr(encoded.c_str()), encoded.size(), v, decoded_size);
}
...
void FastAppendUnsignedVarIntToStr(uint64_t v, std::string* dest) {
  ...
  size_t len = 0;
  FastEncodeUnsignedVarInt(v, fast_varint_detail::to_uchar_ptr(buf), &len);
  DCHECK_LE(len, 10);
  dest->append(buf, len);
}
