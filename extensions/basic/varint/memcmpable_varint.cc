...
#include "basic/varint/memcmpable_varint.h" // IWYU pragma: associated
...
void PutMemcmpableVarint64(std::string *dst, uint64_t value) {
  ...
  dst->append(memcmpable_varint_detail::to_char_ptr(buf), used);
}
...
Status GetMemcmpableVarint64(base::StringPiece* input, uint64_t *value) {
  using namespace ::basic::error;

  const uint8_t* data = memcmpable_varint_detail::to_uchar_ptr(input->data());
  size_t size = sqlite4GetVarint64(data, input->size(), value);

  RETURN_ERROR_IF(size <= 0, INVALID_ARGUMENT)
    << "Valid varint couldn't be decoded";
