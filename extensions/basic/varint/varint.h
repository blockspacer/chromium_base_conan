...
class VarInt {
 public:
  ...
  basic::StatusOr<int64_t> ToInt64() const;

  // The input is expected to be of the form (-)?[0-9]+, whitespace is not allowed. Use this
  // after removing whitespace.
  basic::Status FromString(const std::string& input) {
    return FromString(input.c_str());
  }

  basic::Status FromString(const char* cstr);

  static basic::StatusOr<VarInt> CreateFromString(const std::string& input) {
  ...
  static basic::StatusOr<VarInt> CreateFromString(const char(&input)[N]) {
    using namespace basic::error;
    VarInt result;
  ...
  basic::Status DecodeFromComparable(
      const base::StringPiece& slice, size_t *num_decoded_bytes, size_t num_reserved_bits = 0);

  basic::Status DecodeFromComparable(
      const std::string &string, size_t *num_decoded_bytes, size_t num_reserved_bits = 0) {
    return DecodeFromComparable(base::StringPiece(string), num_decoded_bytes, num_reserved_bits);
  }

  basic::Status DecodeFromComparable(const base::StringPiece& string);
  basic::Status DecodeFromComparable(const std::string& string);
  ...
  basic::Status DecodeFromTwosComplement(const std::string& string);
