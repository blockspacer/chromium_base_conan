...
using stats_hash_map = std::unordered_map<K, V, Hash, KeyEqual>;
...
class BASE_EXPORT StatsTable {
 public:
  ...
  // returns 0.
  int RegisterCurrentThread(const std::string& name);
  ...
  // returns 0.
  int FindOrAddCounter(const std::string& name);
  ...
  static int* FindOrAddLocation(const char *name);

 private:
  class Private;
  struct TLSData;
  typedef stats_hash_map<std::string, int> CountersMap;
  ...
  int FindOrAddCounterOrEmptyRow(const std::string& name) const;
