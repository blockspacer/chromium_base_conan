...
TEST_F(HdrHistogramTest, PercentileAndCopyTest) {
  ...
  ASSERT_NO_FATAL_FAILURE(validate_percentiles(&hist, specified_max));

  HdrHistogram copy(hist);
  ASSERT_NO_FATAL_FAILURE(validate_percentiles(&copy, specified_max));
