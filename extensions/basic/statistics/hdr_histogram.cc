...
// under the License.
//
#include "basic/statistics/hdr_histogram.h" // IWYU pragma: associated
...
// static
const uint64_t HdrHistogram::kMinHighestTrackableValue = 2;

// static
const int HdrHistogram::kMinValidNumSignificantDigits = 1;

// static
const int HdrHistogram::kMaxValidNumSignificantDigits = 5;
...
void HdrHistogram::Init() {
  ...
  CHECK(IsValidHighestTrackableValue(highest_trackable_value_)) <<
      Substitute("highest_trackable_value must be >= $1", kMinHighestTrackableValue);
  CHECK(IsValidNumSignificantDigits(num_significant_digits_)) <<
      Substitute("num_significant_digits must be between $1 and $2",
  ...
  int sub_bucket_count_magnitude =
      Log2Ceiling(largest_value_with_single_unit_resolution);
...
void HdrHistogram::IncrementBy(int64_t value, int64_t count) {
  ...
  {
    Atomic64 min_val;
    while (UNLIKELY(value < (min_val = MinValue()))) {
      Atomic64 old_val = NoBarrier_CompareAndSwap(&min_value_, min_val, value);
      if (LIKELY(old_val == min_val)) break; // CAS success.
    }
  }
  ...
  {
    Atomic64 max_val;
    while (UNLIKELY(value > (max_val = MaxValue()))) {
      Atomic64 old_val = NoBarrier_CompareAndSwap(&max_value_, max_val, value);
      if (LIKELY(old_val == max_val)) break; // CAS success.
    }
  }
}
...
int HdrHistogram::BucketIndex(uint64_t value) const {
  if (UNLIKELY(value > highest_trackable_value_)) {
  ...
  int pow2ceiling = Log2Ceiling64(value | sub_bucket_mask_);
...
double HdrHistogram::MeanValue() const {
  ...
  if (UNLIKELY(count == 0)) return 0.0;
...
basic::Status AbstractHistogramIterator::Next(HistogramIterationValue* value) {
  RETURN_ERROR_IF(histogram_->CurrentCount() != histogram_total_count_)
  ...
  RETURN_ERROR() << "Histogram array index out of bounds while traversing";
}
