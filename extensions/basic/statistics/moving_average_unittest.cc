...
#include "basic/statistics/moving_average.h"
...
TEST(MovingAverageTest, EmptyAverage) {
  basic::MovingAverage moving_average(1);
...
TEST(MovingAverageTest, GetAverage) {
  basic::MovingAverage moving_average(1024);
...
TEST(MovingAverageTest, Reset) {
  basic::MovingAverage moving_average(5);
  moving_average.AddSample(1);
...
TEST(MovingAverageTest, ManySamples) {
  basic::MovingAverage moving_average(10);
  for (int i = 1; i < 11; i++) {
