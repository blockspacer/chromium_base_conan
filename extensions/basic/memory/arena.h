...
class Arena {
 public:
  explicit Arena(
    ...
    if ((maxAlign_ & (maxAlign_ - 1)) || maxAlign_ > alignof(Block)) {
      CHECK(false) << "(Arena) Invalid maxAlign: " << maxAlign_;
    }
  }
 ...
 private:
  ...
  size_t roundUp(size_t size) const {
    size_t maxAl = maxAlign_ - 1;
    base::CheckedNumeric<size_t> realSize = maxAl;
    realSize += size;

    if (!realSize.IsValid()) {
      CHECK(false) << "(Arena) bad allocation";
    }
    return static_cast<size_t>(realSize.ValueOrDie()) & ~maxAl;
  }
...
#include <basic/memory/arena-inl.h>
