...
void* Arena<Alloc>::allocateSlow(size_t size) {
  ...
  base::CheckedNumeric<size_t> checkedSize = sizeof(Block);
  checkedSize += std::max(size, minBlockSize());

  if (!checkedSize.IsValid()) {
    CHECK(false) << "(arena) bad_alloc";
  }

  size_t allocSize = static_cast<size_t>(checkedSize.ValueOrDie());
...
void Arena<Alloc>::merge(Arena<Alloc>&& other) {
  CHECK(blockGoodAllocSize() == other.blockGoodAllocSize())
