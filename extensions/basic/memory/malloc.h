 ...
 */
#if (defined(USE_JEMALLOC) || defined(BASIC_USE_JEMALLOC)) && !BASIC_SANITIZE
...
#include <basic/portability/malloc_impl.h>
...
#define BASIC_MALLOC_CHECKED_MALLOC \
 ...
 */
#if defined(BASIC_ASSUME_NO_JEMALLOC) || BASIC_SANITIZE
  inline bool usingJEMalloc() noexcept {
    return false;
  }
#elif defined(USE_JEMALLOC) && !BASIC_SANITIZE
  inline bool usingJEMalloc() noexcept {
    return true;
  }
#else
BASIC_NOINLINE inline bool usingJEMalloc() noexcept {
...
#if defined(BASIC_ASSUME_NO_TCMALLOC) || BASIC_SANITIZE
  inline bool usingTCMalloc() noexcept {
    return false;
  }
#elif defined(USE_TCMALLOC) && !BASIC_SANITIZE
  inline bool usingTCMalloc() noexcept {
    return true;
  }
#else
BASIC_NOINLINE inline bool usingTCMalloc() noexcept {
  static const bool result = []() noexcept {
    // Some platforms (*cough* OSX *cough*) require weak symbol checks to be
    // in the form if (mallctl != nullptr). Not if (mallctl) or if (!mallctl)
    // (!!). http://goo.gl/xpmctm
...
BASIC_NOINLINE inline bool canSdallocx() noexcept {
  static bool rv = usingJEMalloc() || usingTCMalloc();
  return rv;
}

BASIC_NOINLINE inline bool canNallocx() noexcept {
  static bool rv = usingJEMalloc() || usingTCMalloc();
  return rv;
}
 ...
 */
BASIC_MALLOC_CHECKED_MALLOC BASIC_NOINLINE inline void* smartRealloc(
    void* p,
