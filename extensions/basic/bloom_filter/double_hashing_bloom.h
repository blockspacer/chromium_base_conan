...
#include <basic/hash/city_hash.h>
...
class BloomKeyProbe {
 public:
  ...
  explicit BloomKeyProbe(const base::span<const uint8_t>& key) : key_(key) {
    uint64_t h = basic::CityHash64(key);
  ...
  const base::span<const uint8_t>& key() const { return key_; }
...
class BloomFilterBuilder {
 public:
  ...
  const base::span<const uint8_t> slice() const {
    return base::span<const uint8_t>(&bitmap_[0], n_bytes());
  }
...
inline void BloomFilterBuilder::AddKey(const BloomKeyProbe &probe) {
  ...
  for (size_t i = 0; i < n_hashes_; i++) {
    ...
    bloom_detail::BitmapSet(&bitmap_[0], bitpos);
    h = probe.MixHash(h);
  }
  n_inserted_++;
}
...
inline bool BloomFilter::MayContainKey(const BloomKeyProbe &probe) const {
  ...
  while (rem_hashes >= 2) {
    ...
    if (!bloom_detail::BitmapTest(&bitmap_[0], bitpos1) ||
        !bloom_detail::BitmapTest(&bitmap_[0], bitpos2)) {
      return false;
    }
  ...
  while (rem_hashes) {
    ...
    if (!bloom_detail::BitmapTest(&bitmap_[0], bitpos)) {
      return false;
    }
    h = probe.MixHash(h);
    rem_hashes--;
