...
class FilterBitsBuilder {
 public:
  ...
  virtual void AddKey(const base::StringPiece& key) = 0;

  // Generate the filter using the keys that are added
  // The return value of this function would be the filter bits,
  // The ownership of actual data is set to buf
  virtual base::StringPiece Finish(std::unique_ptr<const char[]>* buf) = 0;
...
// It should be initialized by base::StringPiece generated by BitsBuilder
class FilterBitsReader {
 public:
  virtual ~FilterBitsReader() {}

  // Check if the entry match the bits in filter
  virtual bool MayMatch(const base::StringPiece& entry) = 0;
};
...
// Would first try using functions in Set 2. if they return nullptr,
...
class FilterPolicy {
 public:
  ...
  virtual void CreateFilter(const base::StringPiece* keys, int n, std::string* dst)
      const = 0;
  ...
  virtual bool KeyMayMatch(const base::StringPiece& key, const base::StringPiece& filter) const = 0;
  ...
  // The input base::StringPiece should NOT be deleted by FilterPolicy.
  virtual FilterBitsReader* GetFilterBitsReader(const base::StringPiece& contents) const {
    return nullptr;
  }
  ...
  class KeyTransformer {
   public:
    ...
    // Transform a key.
    virtual base::StringPiece Transform(base::StringPiece key) const = 0;
  };
