...
#include "basic/bloom_filter/double_hashing_bloom.h"
...
inline const uint8_t* unsafe_to_uchar_ptr(const uint64_t *ptr) {
  /// \note *ptr may be greater than std::numeric_limits<uint8_t>::max()
  /// It is ok because unsigned integer overflow is defined behavior
...
static void AddRandomKeys(int random_seed, int n_keys, BloomFilterBuilder *bf) {
  ...
  for (int i = 0; i < n_keys; i++) {
    uint64_t key = random();
    base::span<const uint8_t> key_slice(unsafe_to_uchar_ptr(&key), sizeof(key));
