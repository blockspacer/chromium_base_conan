...
#include "basic/bloom_filter/bloom.h"
...
static const int32_t kBitsPerKey = 10;

static const int kVerbose = 1;

static base::StringPiece Key(size_t i, char* buffer) {
  memcpy(buffer, &i, sizeof(i));
  return base::StringPiece(buffer, sizeof(i));
}
...
class BloomTest : public testing::Test {
 ...
 public:
  BloomTest() : policy_(
      NewBloomFilterPolicy(kBitsPerKey)) {}
  ...
  void Add(const base::StringPiece& s) {
    keys_.push_back(s.as_string());
  }

  void Build() {
    std::vector<base::StringPiece> key_slices;
    for (size_t i = 0; i < keys_.size(); i++) {
      key_slices.push_back(base::StringPiece(keys_[i]));
    }
    filter_.clear();
  ...
  bool Matches(const base::StringPiece& s) {
    if (!keys_.empty()) {
      Build();
    }
...
TEST_F(BloomTest, VaryingLengths) {
  ...
  for (size_t length = 1; length <= 10000; length = NextLength(length)) {
    ...
    if (kVerbose >= 1) {
      LOG(INFO) << basic::Substitute(
          "False positives: $1 @ length = $2 ; bytes = $3\n", rate * 100.0, length,
          FilterSize());
    }
  ...
  if (kVerbose >= 1) {
    LOG(INFO) << basic::Substitute("Filters: $1 good, $2 mediocre\n", good_filters, mediocre_filters);
  }
...
class FullFilterBloomTestContext : public BloomTestContext {
 ...
 private:
  std::unique_ptr<const FilterPolicy> filter_policy_{
      NewBloomFilterPolicy(kBitsPerKey, false)};
};
...
class BuilderReaderBloomTest : public testing::Test,
 ...
 public:
  ...
  void Build() {
    base::StringPiece filter = bits_builder_->Finish(&buf_);
...
INSTANTIATE_TEST_SUITE_P(, BuilderReaderBloomTest, ::testing::Values(
