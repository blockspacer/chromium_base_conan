...
#include <basic/containers/flat_combining_priority_queue.h>
...
bool FLAGS_bench = false; // "run benchmark"
uint32_t FLAGS_reps = 10; // "number of reps"
uint32_t FLAGS_ops = 100000; // "number of operations per rep"
uint32_t FLAGS_size = 64; // "initial size of the priority queue"
uint32_t FLAGS_work = 1000; // "amount of unrelated work per operation"
...
using FCPQ = basic::FlatCombiningPriorityQueue<int>;
  ...
  for (auto n : nthr) {
    ...
    auto fn = [&](uint32_t tid) {
      for (int i = tid; i < ops; i += nthreads) {
        ...
        int v;
        CHECK(pq.try_pop(v));
        EXPECT_NE(pq.try_pop_until(when), base::nullopt);
        pq.pop(v);
        doWork(work);
      }
    };
...
static uint64_t test(std::string name, Exp exp, uint64_t base) {
  ...
  for (int r = 0; r < (int)FLAGS_reps; ++r) {
    uint64_t dur;
    switch (exp) {
      case NoFC: {
        Baseline pq;
  ...
  uint64_t avg = sum / (int)FLAGS_reps;
  uint64_t res = min;
  std::cout << name;
...
/*
$ numactl -N 1 basic/experimental/test/fc_pri_queue_test --bench
