...
#include <basic/containers/mpmc_queue.h>
...
//#include <basic/portability/SysTime.h>
...
//BASIC_ASSUME_FBVECTOR_COMPATIBLE_1(boost::intrusive_ptr)
...
void run_mt_sequencer_test(int numThreads, int numOps, uint32_t init) {
  ...
  int prev = -1;
  vector<std::unique_ptr<basic::test::DSchedThreadWrapper>> threads(numThreads);
...
string producerConsumerBench(
  ...
  vector<std::unique_ptr<basic::test::DSchedThreadWrapper>> producers(numProducers);
  ...
  vector<std::unique_ptr<basic::test::DSchedThreadWrapper>> consumers(numConsumers);
...
void runMtProdConsEmulatedFutex() {
  ...
  const int n = 100000 / (basic::kIsSanitizeThread ? 10 : 1);
  vector<unique_ptr<WriteMethodCaller<QueueType>>> callers;
  callers.emplace_back(std::make_unique<BlockingWriteCaller<QueueType>>());
  callers.emplace_back(std::make_unique<WriteIfNotFullCaller<QueueType>>());
  callers.emplace_back(std::make_unique<WriteCaller<QueueType>>());
...
TEST(MPMCQueue, try_write_until_timeout) {
  basic::MPMCQueue<int, std::atomic, false> queue(1);
  testTimeout<false>(queue);
}

TEST(MPMCQueue, must_fail_try_write_until_dynamic) {
  basic::MPMCQueue<int, std::atomic, true> queue(200, 1, 2);
