...
class FlatCombiningPriorityQueue
 ...
 public:
  ...
  bool empty() const {
    bool res;
    base::RepeatingCallback<void()> fn = base::BindRepeating([](bool& res, const PriorityQueue& pq_) {
      res = pq_.empty();
    }, REFERENCED(res), CONST_REFERENCED(pq_));
    const_cast<FCPQ*>(this)->requestFC(std::move(fn));
    return res;
  }
  ...
  size_t size() const {
    size_t res;
    base::RepeatingCallback<void()> fn = base::BindRepeating([](size_t& res, const PriorityQueue& pq_) {
      res = pq_.size();
    }, REFERENCED(res), CONST_REFERENCED(pq_));
    const_cast<FCPQ*>(this)->requestFC(std::move(fn));
    return res;
  ...
  template <typename Clock, typename Duration>
  base::Optional<T> try_peek_until(
      const std::chrono::time_point<Clock, Duration>& deadline) {
    T val;
 ...
 private:
  ...
  void peek_futex(bool& res, T& val);
  void pop_futex(bool& res, bool& wake, T& val);
  void push_futex(bool& res, bool& wake, const T& val);
};
...
template <
    ...
    class Atom>
inline void
FlatCombiningPriorityQueue<T, PriorityQueue, Mutex, Atom>::push_futex(
  bool& res, bool& wake, const T& val)
{
  ...
  /// \todo return Status to check alloc failure
#if TODO
  try {
#endif
    pq_.push(val);
...
FlatCombiningPriorityQueue<T, PriorityQueue, Mutex, Atom>::try_push_impl(
  ...
  while (true) {
    ...
    base::RepeatingCallback<void()> fn = base::BindRepeating(
      &FlatCombiningPriorityQueue<T, PriorityQueue, Mutex, Atom>::push_futex,
      base::Unretained(this), REFERENCED(res), REFERENCED(wake), CONST_REFERENCED(val));
    this->requestFC(std::move(fn));
...
template <
    ...
    class Atom>
inline void
FlatCombiningPriorityQueue<T, PriorityQueue, Mutex, Atom>::pop_futex(
  bool& res, bool& wake, T& val)
{
  res = !pq_.empty();
  if (res) {
    val = pq_.top();
...
FlatCombiningPriorityQueue<T, PriorityQueue, Mutex, Atom>::try_pop_impl(
  ...
  while (true) {
    ...
    base::RepeatingCallback<void()> fn = base::BindRepeating(
      &FlatCombiningPriorityQueue<T, PriorityQueue, Mutex, Atom>::pop_futex,
      base::Unretained(this), REFERENCED(res), REFERENCED(wake), REFERENCED(val));
    this->requestFC(std::move(fn));
...
template <
    ...
    class Atom>
inline void
FlatCombiningPriorityQueue<T, PriorityQueue, Mutex, Atom>::peek_futex(
  bool& res, T& val)
{
  res = !pq_.empty();
   if (res) {
...
FlatCombiningPriorityQueue<T, PriorityQueue, Mutex, Atom>::try_peek_impl(
  ...
  while (true) {
    ...
    base::RepeatingCallback<void()> fn = base::BindRepeating(
      &FlatCombiningPriorityQueue<T, PriorityQueue, Mutex, Atom>::peek_futex,
      base::Unretained(this), REFERENCED(res), REFERENCED(val));
    this->requestFC(std::move(fn));
