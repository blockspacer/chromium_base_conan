...
#include "basic/containers/unique_any.h"
...
TEST(UniqueAnyTest, MakeAny) {
  const CopyOnlyInt copy_only{};
  auto o = basic::make_unique_any<IntMoveOnlyCopyOnlyInt>(5, MoveOnlyInt(),
                                                         copy_only);
...
TEST(UniqueAnyTest, MakeAnyIList) {
  const CopyOnlyInt copy_only{};
  auto o = basic::make_unique_any<ListMoveOnlyCopyOnlyInt>(
  ...
  basic::unique_any a = basic::make_unique_any<std::vector<int>>({1, 2, 3, 4});
