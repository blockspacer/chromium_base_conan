...
// basic::unique_any is similar to std::any except it:
...
// basic::unique_any a = 123;
// EXPECT_EQ(123, basic::unique_any_cast<int>(a));
//
// a = std::string("123");
// EXPECT_EQ("123", basic::unique_any_cast<std::string>(a));
//
// a = make_unique_ptr("123");
// EXPECT_EQ("123", *basic::unique_any_cast<std::unique_ptr<int>>(a));
//
// basic::unique_any b(std::move(a));
// EXPECT_EQ("123", *basic::unique_any_cast<std::unique_ptr<int>>(b));
//
// Note an incorrect basic::unique_any_cast will lead to a CHECK.
...
// Constructs a basic::unique_any of type |T| with the given arguments.
template <typename T, typename... Args>
unique_any make_unique_any(Args&&... args);

// Overload of |base::make_unique_any()| for constructing a basic::unique_any
// type from an initializer list. E.g.
// basic::unique_any a = base::make_any<std::vector<int>>({1, 2, 3, 4});
template <typename T, typename U, typename... Args>
unique_any make_unique_any(std::initializer_list<U> il, Args&&... args);

// Statically casts the value of a const basic::unique_any to the given type.
...
//
//     basic::unique_any a = 123;
//     int i = basic::unique_any_cast<int>(a);  // i = 123
//
...
//
//     basic::unique_any my_any = std::vector<int>();
//     basic::unique_any_cast<std::vector<int>&>(my_any).push_back(42);
//
...
// basic::unique_any to the given pointer type, or nullptr if the stored value
...
class BASE_EXPORT unique_any {
 ...
 public:
  // Constructs an empty basic::unique_any.
  constexpr unique_any() noexcept {}

  // Constructs a basic::unique_any with the value contained by |other| moved
  // into it.
  ...
  // Constructs a basic::unique_any containing |value| as long as |T| isn't
  // basic::unique_any nor base::in_place_type_t<> and |T| is move or copy
  // constructible. E.g. basic::unique_any a(123);
  template <
      typename T,
      typename VT = std::decay_t<T>,
      std::enable_if_t<!internal::is_unique_any<VT>::value>* = nullptr,
      std::enable_if_t<!base::is_in_place_type_t<VT>::value>* = nullptr,
  ...
  // Constructs a basic::unique_any containing an object of type T which is
  // initialized by std::forward<Args>(args). E.g.
  // basic::unique_any a(base::in_place_type_t<std::unique_ptr<int>>(), 123);
  template <
      typename T,
      typename... Args,
      typename VT = std::decay_t<T>,
  ...
  explicit unique_any(base::in_place_type_t<T> /*tag*/, Args&&... args) noexcept
  ...
  // remaining args. E.g. basic::unique_any
  // a(base::in_place_type_t<std::vector<int>>(), {1, 2, 3}); basic::unique_any
  ...
  template <typename T,
            ...
            std::enable_if_t<is_move_or_copy_constructible<VT>::value &&
                             std::is_constructible<VT,
                                                   ...
                                                   Args...>::value>* = nullptr>
  explicit unique_any(base::in_place_type_t<T> /*tag*/,
  ...
  // Emplaces a value within a basic::unique_any object by calling reset(),
  ...
  // Overload of |emplace()| to emplace a value within a |basic::unique_any|
...
template <typename T, typename... Args>
unique_any make_unique_any(Args&&... args) {
  return unique_any(base::in_place_type_t<T>(), std::forward<Args>(args)...);
}

template <typename T, typename U, typename... Args>
unique_any make_unique_any(std::initializer_list<U> il, Args&&... args) {
  return unique_any(base::in_place_type_t<T>(), il, std::forward<Args>(args)...);
}
