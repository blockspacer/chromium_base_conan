 ...
 public:
  ...
  class Pointer {
   public:
    ...
    // Comparing to Pointer from a different PrioritizedList is undefined.
   ...
   private:
    ...
    typedef typename PrioritizedList::List::iterator ListIterator;
  ...
  explicit PrioritizedList(Priority num_priorities)
  ...
  PrioritizedList(const PrioritizedList&) = delete;
  PrioritizedList& operator=(const PrioritizedList&) = delete;
  ~PrioritizedList() { DCHECK_CALLED_ON_VALID_THREAD(thread_checker_); }
  ...
  // |rhs|. Assumes that both Pointers refer to elements in this PrioritizedList.
