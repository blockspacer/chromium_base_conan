...
class Baton {
 public:
  ...
  ALWAYS_INLINE bool try_wait() const noexcept { return ready(); }
 ...
 private:
  ...
  template <typename Clock, typename Duration>
  NOINLINE bool tryWaitSlow(
      const std::chrono::time_point<Clock, Duration>& deadline,
      const WaitOptions& opt) noexcept {
    if (opt.logging_enabled()) {
      VLOG(2)
        << "(Baton) Blocking operation, milliseconds: "
    ...
    if (!basic::atomic_compare_exchange_strong_explicit<Atom>(
            &state_,
            &expected,
