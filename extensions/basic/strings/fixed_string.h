...
#define FIXED_STRING_HAS_STRING_VIEW 1

#if FIXED_STRING_HAS_STRING_VIEW
#include <string_view>
#endif
...
[[noreturn]] inline void assertOutOfBounds() {
  CHECK(false) << "Array index out of bounds in BasicFixedString";
}
...
constexpr const Char (&checkNullTerminated(const Char (&a)[N]) noexcept)[N] {
  ...
  return a[N - 1u] == Char(0)
          ...
          && (!kIsDebug || N - 1u == basic::constexpr_strlen(a))
      ? decltype(a)(a)
 ...
 * Instead of using the `basic::makeFixedString` helper function, you can use
 ...
 * \par
 * \code
 * using namespace basic::string_literals;
 ...
 *  -# Otherwise, fail with out of range
 *\par
...
class BasicFixedString : private detail::fixedstring::FixedStringBase {
 private:
  ...
  constexpr BasicFixedString(
      ...
      : data_{(Is < (size - pos) && Is < count ? that[Is + pos] : Char(0))..., Char(0)},
        size_{basic::constexpr_min(size - pos, count)} {}
 ...
 public:
  ...
  /** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** ** **
   * Conversion to base::span
   * \return `base::span<Char*>{begin(), end()}`
   */
  constexpr base::span<Char*> toSpan() noexcept { return {begin(), end()}; }

  /**
   * \overload
   */
  constexpr base::span<const Char*> toSpan() const noexcept {
    return {begin(), end()};
  }
  ...
  base::StringPiece toStringPiece() const noexcept(false) {
    return base::StringPiece{toStdString()};
  }

  /// \todo support base::StringPiece16
  /* implicit */ constexpr operator base::StringPiece() const {
    return base::StringPiece{begin(), size()};
  }
  ...
  // API that has overloads for `const char*` and `base::span`, for instance.
  ...
  constexpr void swap(BasicFixedString& that) noexcept {
    ...
    for (std::size_t i = 0u; i <= basic::constexpr_max(size_, that.size_);
         ++i) {
  ...
  constexpr Char& at(std::size_t i) noexcept(false) {
    if(i > size_) CHECK(false) << "Out of range in BasicFixedString::at";
  ...
  constexpr BasicFixedString& append(const Char* that) noexcept(false) {
    return append(that, basic::constexpr_strlen(that));
  }
  ...
  constexpr int compare(const Char* that) const noexcept {
    return compare(0u, size_, that, basic::constexpr_strlen(that));
  }

  /**
   * \overload
   */
  constexpr int compare(base::StringPiece that) const noexcept {
  ...
  constexpr int compare(
      ...
      noexcept(false) {
    return compare(this_pos, this_count, that, basic::constexpr_strlen(that));
  }
  ...
  constexpr int compare(
      std::size_t this_pos,
      std::size_t this_count,
      base::StringPiece that) const noexcept(false) {
  ...
  constexpr BasicFixedString& replace(
    ...
    return replace(this_pos, this_count, that, basic::constexpr_strlen(that));
  }
  ...
  constexpr BasicFixedString& replace(
    ...
    return replace(
        first - data_, last - first, that, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find(const Char* that) const noexcept {
    return find(that, 0u, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find(const Char* that, std::size_t pos) const
      noexcept(false) {
    return find(that, pos, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t rfind(const Char* that) const noexcept {
    return rfind(that, size_, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t rfind(const Char* that, std::size_t pos) const
      noexcept(false) {
    return rfind(that, pos, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find_first_of(const Char* that) const noexcept {
    return find_first_of(that, 0u, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find_first_of(const Char* that, std::size_t pos) const
      noexcept(false) {
    return find_first_of(that, pos, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find_first_not_of(const Char* that) const noexcept {
    return find_first_not_of(that, 0u, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find_first_not_of(
    ...
    return find_first_not_of(that, pos, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find_last_of(const Char* that) const noexcept {
    return find_last_of(that, size_, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find_last_of(const Char* that, std::size_t pos) const
      noexcept(false) {
    return find_last_of(that, pos, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find_last_not_of(const Char* that) const noexcept {
    return find_last_not_of(that, size_, basic::constexpr_strlen(that));
  }
  ...
  constexpr std::size_t find_last_not_of(
      const Char* that, std::size_t pos) const noexcept(false) {
    return find_last_not_of(that, pos, basic::constexpr_strlen(that));
  }
  ...
  friend constexpr bool operator==(
    ...
    return detail::fixedstring::equal_(
        a, basic::constexpr_strlen(a), b.data_, b.size_);
  }
   ...
   */
  friend constexpr bool operator==(
      base::StringPiece a, const BasicFixedString& b) noexcept {
  ...
  friend constexpr bool operator<(
    ...
    return ordering::lt ==
        detail::fixedstring::compare_(
               a, 0u, basic::constexpr_strlen(a), b.data_, 0u, b.size_);
  }
  ...
  friend constexpr bool operator<(
    ...
    return ordering::lt ==
        detail::fixedstring::compare_(
               a.data_, 0u, a.size_, b, 0u, basic::constexpr_strlen(b));
  }
   ...
   */
  friend constexpr bool operator<=(
      base::StringPiece const& a, const BasicFixedString& b) noexcept {
    return !(b < a);
  }
   ...
   */
  friend constexpr bool operator>=(
      const BasicFixedString& a, base::StringPiece const& b) noexcept {
    return !(a < b);
  }
...
inline namespace {
// "const std::size_t&" is so that basic::npos has the same address in every
...
#define BASIC_DEFINE_FIXED_STRING_UDL(N)                     \
...
BASIC_DEFINE_FIXED_STRING_UDL(4)
BASIC_DEFINE_FIXED_STRING_UDL(8)
BASIC_DEFINE_FIXED_STRING_UDL(16)
BASIC_DEFINE_FIXED_STRING_UDL(32)
BASIC_DEFINE_FIXED_STRING_UDL(64)
BASIC_DEFINE_FIXED_STRING_UDL(128)

#undef BASIC_DEFINE_FIXED_STRING_UDL
