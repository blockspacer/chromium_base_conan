...
// `is_boolean` in substitute is not limited to `bool`.
// You can add custom types here that can be converted
// to strings "true" and "false"
namespace detail
{
  template <typename T>
...
class SubstituteArg {
 public:
  ...
  explicit SubstituteArg(const char* value);

  // Overload `const string&` to avoid a temporary object.
  explicit SubstituteArg(const std::string& value);

  explicit SubstituteArg(std::string&& value);

  // Overload `const base::string16&` to avoid a temporary object.
  explicit SubstituteArg(const base::string16& value);

  explicit SubstituteArg(const base::StringPiece& value);

  explicit SubstituteArg(const base::StringPiece16& value);

  SubstituteArg(const base::Location& value);
  ...
  SubstituteArg(const void* bytes);
...
class SubstituteArg16 {
 public:
  ...
  // Overload `const char*` because compiler casts it to bool.
  explicit SubstituteArg16(const char* value);

  // Overload `const string&` to avoid a temporary object.
  explicit SubstituteArg16(const std::string& value);

  // Overload `const base::string16&` to avoid a temporary object.
  explicit SubstituteArg16(const base::string16& value);

  explicit SubstituteArg16(base::string16&& value);

  explicit SubstituteArg16(const base::StringPiece& value);

  explicit SubstituteArg16(const base::StringPiece16& value);

  SubstituteArg16(const base::Location& value);
  ...
  SubstituteArg16(const void* bytes);
...
void SubstituteAndAppend(
  base::string16* output, const base::string16& format,
...
void SubstituteAndAppend(
  std::string* output, base::StringPiece format,
