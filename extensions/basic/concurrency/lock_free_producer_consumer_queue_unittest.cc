...
#include <basic/concurrency/lock_free_producer_consumer_queue.h>
...
void perfTestType(const char* type) {
  ...
  LOG(INFO) << "Type: " << type;
  doTest<PerfTest<basic::LockFreeProducerConsumerQueue<T>, size, Pop>>(
...
void correctnessTestType(const std::string& type) {
  LOG(INFO) << "Type: " << type;
  doTest<CorrectnessTest<basic::LockFreeProducerConsumerQueue<T>, 0xfffe, Pop>>(
      "ProducerConsumerQueue");
}
...
TEST(PCQ, Destructor) {
  ...
  // destroyed.
  {
    basic::LockFreeProducerConsumerQueue<DtorChecker> queue(1024);
    for (int i = 0; i < 10; ++i) {
  ...
  {
    basic::LockFreeProducerConsumerQueue<DtorChecker> queue(4);
    for (int i = 0; i < 3; ++i) {
      EXPECT_TRUE(queue.write(DtorChecker()));
    }
...
TEST(PCQ, EmptyFull) {
  basic::LockFreeProducerConsumerQueue<int> queue(3);
