...
struct SequentialThreadId {
 ...
 private:
  ...
  static BASIC_TLS unsigned currentId;
};
...
template <template <typename> class Atom>
BASIC_TLS unsigned SequentialThreadId<Atom>::currentId(0);
...
#ifdef BASIC_CL_USE_BASIC_TLS
  static BASIC_TLS CpuCache cpuCache;
#endif
   ...
   public:
    void* allocate(size_t size) {
      ...
      if (cl == 4) {
        ...
        void* mem =
            base::AlignedAlloc(size, hardware_destructive_interference_size);
        CHECK(mem);
        return mem;
      }
...
getCoreAllocator(size_t stripe) {
  ...
  static base::NoDestructor<CoreRawAllocator<Stripes>> allocator;
