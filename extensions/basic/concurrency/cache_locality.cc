...
static CacheLocality getSystemLocalityInfo() {
  ...
  int numCpus = base::SysInfo::NumberOfProcessors();
  if (numCpus <= 0) {
...
static size_t parseLeadingNumber(const std::string& line) {
  ...
  if (end == raw || (*end != ',' && *end != '-' && *end != '\n' && *end != 0)) {
    CHECK(false) << "error parsing list '" << line << "'";
  }
  return val;
}
...
CacheLocality CacheLocality::readFromSysfsTree(
  ...
  while (true) {
    ...
    for (size_t index = 0;; ++index) {
      auto dir =
          basic::fmt("/sys/devices/system/cpu/cpu{0}/cache/index{1}/", cpu, index);
  ...
  if (cpus.empty()) {
    CHECK(false) << "unable to load cache sharing info";
  }
...
CacheLocality CacheLocality::readFromProcCpuinfoLines(
  ...
  if (cpus.empty()) {
    CHECK(false) << "no CPUs parsed from /proc/cpuinfo";
  }
...
CacheLocality CacheLocality::readFromProcCpuinfo() {
  std::vector<std::string> lines;
  {
    ...
    if (xi.fail()) {
      CHECK(false) << "unable to open /proc/cpuinfo";
    }
    char buf[8192];
...
#if !defined(BASIC_HAVE_LINUX_VDSO) || defined(BASIC_SANITIZE_MEMORY)
  return nullptr;
#else
...
SimpleAllocator::~SimpleAllocator() {
  ...
  for (auto& block : blocks_) {
    base::AlignedFree(block);
  }
}

void* SimpleAllocator::allocateHard() {
  // Allocate a new slab.
  mem_ = static_cast<uint8_t*>(base::AlignedAlloc(allocSize_, allocSize_));
  CHECK(mem_);
