...
using basic_port_struct_timezone = struct timezone;

#else

// Someone decided this was a good place to define timeval.....
#include <basic/portability/windows.h>

struct basic_port_struct_timezone_ {
  int tz_minuteswest;
  int tz_dsttime;
};
using basic_port_struct_timezone = struct basic_port_struct_timezone_;

extern "C" {

// We use basic_port_struct_timezone due to issues with #defines on Windows
// platforms.
...
// Windows.  Instead users should use basic_port_struct_timezone instead.
int gettimeofday(timeval* tv, basic_port_struct_timezone*);
