...
template <typename... T>
BASIC_INLINE_VARIABLE constexpr tag_t<T...> tag{};
...
BASIC_INLINE_VARIABLE constexpr bool is_instantiation_of_v = false;
template <template <typename...> class C, typename... T>
BASIC_INLINE_VARIABLE constexpr bool is_instantiation_of_v<C, C<T...>> = true;
template <template <typename...> class C, typename... T>
struct is_instantiation_of : bool_constant<is_instantiation_of_v<C, T...>> {};

template <typename, typename>
BASIC_INLINE_VARIABLE constexpr bool is_similar_instantiation_v = false;
...
//  constexpr context.
template <typename T>
BASIC_INLINE_VARIABLE constexpr bool is_constexpr_default_constructible_v =
 ...
 *
 *    template <class T>
 *    struct has_value_type<T, basic::void_t<typename T::value_type>>
 *        : std::true_type {};
 */
...
template <template <typename...> class T, typename... A>
BASIC_INLINE_VARIABLE constexpr bool is_detected_v =
    detected_or<nonesuch, T, A...>::value_t::value;
template <template <typename...> class T, typename... A>
...
template <class T>
BASIC_INLINE_VARIABLE constexpr bool is_trivially_copyable_v =
 ...
 * namespace basic and make sure you keep the specialization of
...
#define BASIC_HAS_TRUE_XXX(name)                                             \
  template <typename T>                                                      \
...
BASIC_HAS_TRUE_XXX(IsRelocatable);
BASIC_HAS_TRUE_XXX(IsZeroInitializable);

#undef BASIC_HAS_TRUE_XXX
...
template <typename T>
BASIC_INLINE_VARIABLE constexpr bool is_transparent_v =
...
/**
 * Use this macro ONLY inside namespace basic. When using it with a
 * regular type, use it like this:
 *
 * // Make sure you're at namespace ::basic scope
 * template <> BASIC_ASSUME_RELOCATABLE(MyType)
 *
 * When using it with a template type, use it like this:
 *
 * // Make sure you're at namespace ::basic scope
 * template <class T1, class T2>
 * BASIC_ASSUME_RELOCATABLE(MyType<T1, T2>)
 */
#define BASIC_ASSUME_RELOCATABLE(...) \
  struct IsRelocatable<__VA_ARGS__> : std::true_type {}

/**
 * The BASIC_ASSUME_FBVECTOR_COMPATIBLE* macros below encode the
 ...
 *
 * Use BASIC_ASSUME_FBVECTOR_COMPATIBLE with regular types like this:
 *
 * BASIC_ASSUME_FBVECTOR_COMPATIBLE(MyType)
 *
 * The versions BASIC_ASSUME_FBVECTOR_COMPATIBLE_1, _2, _3, and _4
 ...
 * BASIC_ASSUME_FBVECTOR_COMPATIBLE_2(MyType)
 */

// Use this macro ONLY at global level (no namespace)
#define BASIC_ASSUME_FBVECTOR_COMPATIBLE(...) \
  namespace basic {                           \
  template <>                                 \
  BASIC_ASSUME_RELOCATABLE(__VA_ARGS__);      \
  }
// Use this macro ONLY at global level (no namespace)
#define BASIC_ASSUME_FBVECTOR_COMPATIBLE_1(...) \
  namespace basic {                             \
  template <class T1>                           \
  BASIC_ASSUME_RELOCATABLE(__VA_ARGS__<T1>);    \
  }
// Use this macro ONLY at global level (no namespace)
#define BASIC_ASSUME_FBVECTOR_COMPATIBLE_2(...)  \
  namespace basic {                              \
  template <class T1, class T2>                  \
  BASIC_ASSUME_RELOCATABLE(__VA_ARGS__<T1, T2>); \
  }
// Use this macro ONLY at global level (no namespace)
#define BASIC_ASSUME_FBVECTOR_COMPATIBLE_3(...)      \
  namespace basic {                                  \
  template <class T1, class T2, class T3>            \
  BASIC_ASSUME_RELOCATABLE(__VA_ARGS__<T1, T2, T3>); \
  }
// Use this macro ONLY at global level (no namespace)
#define BASIC_ASSUME_FBVECTOR_COMPATIBLE_4(...)          \
  namespace basic {                                      \
  template <class T1, class T2, class T3, class T4>      \
  BASIC_ASSUME_RELOCATABLE(__VA_ARGS__<T1, T2, T3, T4>); \
  }
...
// basic::to integral specializations can end up generating code
...
BASIC_PUSH_WARNING
BASIC_GNU_DISABLE_WARNING("-Wsign-compare")
BASIC_GCC_DISABLE_WARNING("-Wbool-compare")
BASIC_MSVC_DISABLE_WARNING(4287) // unsigned/negative constant mismatch
BASIC_MSVC_DISABLE_WARNING(4388) // sign-compare
BASIC_MSVC_DISABLE_WARNING(4804) // bool-compare
...
#ifndef _MSC_VER
BASIC_ASSUME_FBVECTOR_COMPATIBLE_2(std::unique_ptr)
BASIC_ASSUME_FBVECTOR_COMPATIBLE_1(std::shared_ptr)
#endif
 ...
 * traits.
 *
 * If BASIC_SUPPLY_MISSING_INT128_TRAITS is defined, we define these traits
 * here.
 *
 * @author: Phil Willoughby <philwill@fb.com>
 */
#if BASIC_SUPPLY_MISSING_INT128_TRAITS
BASIC_NAMESPACE_STD_BEGIN
template <>
...
BASIC_NAMESPACE_STD_END
#endif // BASIC_SUPPLY_MISSING_INT128_TRAITS
