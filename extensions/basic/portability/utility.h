 ...
 *      void foo() {
 *        MyData data;
 *        something(basic::copy(data)); // explicit copy
 ...
 *
 *      basic::copy(std::move(data)); // force-move, not just a cast to &&
 *
 ...
 *  public:
 *    explicit Something(int);
 *    Something(basic::initlist_construct_t, std::initializer_list<int>);
 *
 *    operator int();
 *  };
 *
 ...
 *  Something something_two{basic::initlist_construct, {1}}; // correct
 */
...
//  Example:
//
//    void takes_numbers(basic::sorted_unique_t, std::vector<int> alist) {
...
//      takes_numbers(basic::sorted_unique, alist);
//    }
//
...
//  Example:
//
//    void takes_numbers(basic::sorted_equivalent_t, std::vector<int> alist) {
//      assert(std::is_sorted(alist.begin(), alist.end()));
//      for (i : alist) {
...
//    void takes_numbers(std::vector<int> alist) {
//      std::sort(alist.begin(), alist.end());
//      takes_numbers(basic::sorted_equivalent, alist);
//    }
//
...
BASIC_INLINE_VARIABLE constexpr identity_fn identity;
...
BASIC_INLINE_VARIABLE constexpr to_signed_fn to_signed;
...
BASIC_INLINE_VARIABLE constexpr to_unsigned_fn to_unsigned;
...
class to_narrow_convertible {
 public:
  ...
  /* implicit */ constexpr operator Dst() const noexcept {
    BASIC_PUSH_WARNING
    BASIC_MSVC_DISABLE_WARNING(4244) // lossy conversion: arguments
    BASIC_MSVC_DISABLE_WARNING(4267) // lossy conversion: variables
    BASIC_GNU_DISABLE_WARNING("-Wconversion")
    return value_;
    BASIC_POP_WARNING
  }
...
BASIC_INLINE_VARIABLE constexpr to_narrow_fn to_narrow;
...
BASIC_INLINE_VARIABLE constexpr to_underlying_fn to_underlying;
 ...
 *   point of use.
 *   If this is a problem then use BASIC_DECLVAL(T&&) instead, or if T might
 *   be 'void', then use BASIC_DECLVAL(std::add_rvalue_reference_t<T>).
 */
#if __cplusplus >= 201703L
#define BASIC_DECLVAL(...) static_cast<__VA_ARGS__ (*)() noexcept>(nullptr)()
#else
...
#define BASIC_DECLVAL(...) ::basic::detail::declval<__VA_ARGS__>()
#endif
