...
using ::basic::Status;
using ::basic::PosixErrorToStatus;
...
TEST(StatusOr, TestMoveOnlyVector) {
  ...
  EXPECT_EQ(::basic::UnknownStatus(FROM_HERE), another_vec[1].status());
}

TEST(StatusOr, TestDefaultCtor) {
  StatusOr<int> thing;
  EXPECT_FALSE(thing.ok());
  EXPECT_TRUE(thing.status().Matches(::basic::UnknownStatus(FROM_HERE)));
  EXPECT_FALSE(thing.status().Matches(::basic::OkStatus()));
}
...
TEST(StatusOr, TestStatusCtor) {
  StatusOr<int> thing(::basic::CancelledStatus(FROM_HERE));
  EXPECT_FALSE(thing.ok());
  EXPECT_TRUE(thing.status().Matches(::basic::CancelledStatus(FROM_HERE)));
}

TEST(StatusOrDeathTest, TestStatusCtorStatusOk) {
  EXPECT_DEBUG_DEATH({
    // This will DCHECK
    StatusOr<int> thing(::basic::OkStatus());
    // In optimized mode, we are actually going to get EINVAL for
    // status here, rather than crashing, so check that.
    EXPECT_FALSE(thing.ok());
    EXPECT_TRUE(thing.status().Matches(PosixErrorToStatus(FROM_HERE, EINVAL, "")));
  }, "OK Status is not a valid constructor argument");
}
...
TEST(StatusOr, TestCopyCtorStatusNotOk) {
  StatusOr<int> original(::basic::CancelledStatus(FROM_HERE));
  StatusOr<int> copy(original);
  EXPECT_TRUE(copy.status().Matches(original.status()));
}
...
TEST(StatusOr, TestAssignmentStatusNotOk) {
  StatusOr<int> source(::basic::CancelledStatus(FROM_HERE));
  StatusOr<int> target;
  target = source;
  EXPECT_TRUE(target.status().Matches(source.status()));
}
...
TEST(StatusOr, TestStatus) {
  StatusOr<int> good(4);
  EXPECT_TRUE(good.ok());
  StatusOr<int> bad(::basic::CancelledStatus(FROM_HERE));
  EXPECT_FALSE(bad.ok());
  EXPECT_TRUE(bad.status().Matches(::basic::CancelledStatus(FROM_HERE)));
}
...
TEST(StatusOrDeathTest, TestValueNotOk) {
  StatusOr<int> thing(::basic::CancelledStatus(FROM_HERE));
  EXPECT_FALSE(thing.ok());
  EXPECT_DEATH(thing.ValueOrDie(), "Attempting to fetch value instead of handling error.*generic.*cancelled.*");
}

TEST(StatusOrDeathTest, TestValueNotOkConst) {
  const StatusOr<int> thing(::basic::UnknownStatus(FROM_HERE));
  EXPECT_FALSE(thing.ok());
  EXPECT_DEATH(thing.ValueOrDie(), "Attempting to fetch value instead of handling error.*generic.*unknown.*");
}
...
TEST(StatusOr, TestPointerCopyCtorStatusNotOkConverting) {
  StatusOr<Derived*> original(::basic::CancelledStatus(FROM_HERE));
  StatusOr<Base2*> copy(original);
  EXPECT_TRUE(copy.status().Matches(original.status()));
}
...
TEST(StatusOr, TestPointerAssignmentStatusNotOkConverting) {
  StatusOr<Derived*> source(::basic::CancelledStatus(FROM_HERE));
  StatusOr<Base2*> target;
  target = source;
  EXPECT_TRUE(target.status().Matches(source.status()));
}
...
TEST(StatusOr, TestPointerStatus) {
  ...
  EXPECT_TRUE(good.ok());
  StatusOr<const int*> bad(::basic::CancelledStatus(FROM_HERE));
  EXPECT_TRUE(bad.status().Matches(::basic::CancelledStatus(FROM_HERE)));
}
...
TEST(StatusOrDeathTest, TestPointerValueNotOkConst) {
  const StatusOr<int*> thing(::basic::CancelledStatus(FROM_HERE));
  EXPECT_FALSE(thing.ok());
  EXPECT_DEATH(thing.ValueOrDie(), "Attempting to fetch value instead of handling error.*generic.*cancelled.*");
}
...
class BenchmarkFactory {
 public:
  ...
  Status ArgumentFactoryFail(T** result) ABSL_ATTRIBUTE_NOINLINE {
    *result = NULL;
    return ::basic::CancelledStatus(FROM_HERE);
  }
