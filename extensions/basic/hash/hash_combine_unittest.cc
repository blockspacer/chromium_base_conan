...
using namespace basic::hash;

// TEST(Hash, hasher) {
//   // Basically just confirms that things compile ok.
//   std::unordered_map<int32_t, int32_t, basic::hasher<int32_t>> m;
...
//         basic::to<typename M::key_type>(i), typename M::mapped_type{}));
//     ret.push_back(A::getAllocatedMemorySize());
//   }
//   return ret;
// }
...
//   testNoCachedHashCode<bool, char, basic::hasher<bool>>();
//   testNoCachedHashCode<int, char, basic::hasher<int>>();
//   testNoCachedHashCode<double, char, basic::hasher<double>>();
//   testNoCachedHashCode<TestEnum, char, basic::hasher<TestEnum>>();
//
//   testNoCachedHashCode<bool, std::string, basic::Hash>();
//   testNoCachedHashCode<int, std::string, basic::Hash>();
//   testNoCachedHashCode<double, std::string, basic::Hash>();
//   testNoCachedHashCode<TestEnum, std::string, basic::Hash>();
// }

TEST(Hash, integer_conversion) {
  basic::hasher<uint64_t> h;
  uint64_t k = 10;
  EXPECT_EQ(h(k), h(10));
}
...
TEST(Hash, std_empty_tuple) {
  std::unordered_map<std::tuple<>, std::string, basic::Hash> m;
  m[{}] = "foo";
  EXPECT_EQ(m[{}], "foo");

  basic::hasher<std::tuple<>> h;
  EXPECT_EQ(h({}), 0);
}
...
TEST(Hash, enum_type) {
  ...
  std::unordered_map<Enum32, std::string, basic::Hash> m32;
  m32[Enum32::Foo] = "foo";
  ...
  std::unordered_map<Enum64, std::string, basic::Hash> m64;
  m64[Enum64::Foo] = "foo";
  EXPECT_EQ("foo", m64[Enum64::Foo]);
}

TEST(Hash, pair_basic_hash) {
  typedef std::pair<int64_t, int32_t> pair2;
  pair2 p(42, 1);

  std::unordered_map<pair2, std::string, basic::Hash> m;
  m[p] = "bar";
  EXPECT_EQ("bar", m[p]);
}

TEST(Hash, tuple_basic_hash) {
  typedef std::tuple<int64_t, int32_t, int32_t> tuple3;
  tuple3 t(42, 1, 1);

  std::unordered_map<tuple3, std::string, basic::Hash> m;
  m[t] = "bar";
  EXPECT_EQ("bar", m[t]);
}
...
TEST(Hash, commutative_hash_combine) {
  EXPECT_EQ(
      commutative_hash_combine_value_generic(
          basic::Hash{}(12345ul), basic::Hash{}, 6789ul),
      commutative_hash_combine_value_generic(
          basic::Hash{}(6789ul), basic::Hash{}, 12345ul));
  ...
  EXPECT_NE(
      ...
      commutative_hash_combine_range_generic(
          /* seed = */ 0xdeadbeef, basic::Hash{}, v.begin(), v.end()));
  EXPECT_NE(
...
//   using namespace basic;
//
//   base::StringPiece a1 = "10050517", b1 = "51107032", a2 = "10050518",
...
// void pointerTestWithBasicHash() {
//   std::unordered_set<PtrType<std::string>, basic::Hash> set;
//
//   for (auto i = 0; i < 1000; ++i) {
//     auto random = PtrType<std::string>{
//         new std::string{basic::to<std::string>(basic::Random::rand64())}};
...
// TEST(Hash, UniquePtr) {
//   pointerTestWithBasicHash<std::unique_ptr>();
// }
//
// TEST(Hash, SharedPtr) {
//   pointerTestWithBasicHash<std::shared_ptr>();
// }
//
// TEST(Hash, Pointer) {
//   pointerTestWithBasicHash<Pointer>();
//
//   EXPECT_TRUE(
//       (std::is_same<
//           basic::hasher<std::string*>::basic_is_avalanching,
//           basic::hasher<std::unique_ptr<std::string>>::basic_is_avalanching>::
//            value));
//
//   EXPECT_TRUE(
//       (std::is_same<
//           basic::hasher<std::string*>::basic_is_avalanching,
//           basic::hasher<std::shared_ptr<std::string>>::basic_is_avalanching>::
//            value));
// }
...
static_assert(!basic::IsAvalanchingHasher<std::hash<int>, int>::value, "");
static_assert(
    !basic::IsAvalanchingHasher<std::hash<char const*>, char const*>::value,
    "");
static_assert(!basic::IsAvalanchingHasher<std::hash<float>, float>::value, "");
static_assert(
    !basic::IsAvalanchingHasher<std::hash<double>, double>::value, "");
static_assert(
    !basic::IsAvalanchingHasher<std::hash<long double>, long double>::value,
    "");
// static_assert(
//     basic::IsAvalanchingHasher<std::hash<std::string>, std::string>::value, "");
static_assert(
    !basic::IsAvalanchingHasher<std::hash<TestEnum>, TestEnum>::value, "");
static_assert(
    !basic::IsAvalanchingHasher<std::hash<TestStruct>, TestStruct>::value, "");

static_assert(
    !basic::IsAvalanchingHasher<basic::transparent<std::hash<int>>, int>::value,
    "");
static_assert(
    basic::IsAvalanchingHasher<
        basic::transparent<std::hash<std::string>>,
        std::string>::value,
    "");

// these come from basic/hash/Hash.h
static_assert(
    basic::IsAvalanchingHasher<
    ...
    "");
static_assert(
    !basic::IsAvalanchingHasher<std::hash<std::tuple<int>>, std::tuple<int>>::
        value,
    "");
static_assert(
    basic::IsAvalanchingHasher<
...
static_assert(
    k32Bit == basic::IsAvalanchingHasher<basic::Hash, uint8_t>::value, "");
static_assert(
    k32Bit == basic::IsAvalanchingHasher<basic::Hash, char>::value, "");
static_assert(
    k32Bit == basic::IsAvalanchingHasher<basic::Hash, uint16_t>::value, "");
static_assert(
    k32Bit == basic::IsAvalanchingHasher<basic::Hash, int16_t>::value, "");
static_assert(
    k32Bit == basic::IsAvalanchingHasher<basic::Hash, uint32_t>::value, "");
static_assert(
    k32Bit == basic::IsAvalanchingHasher<basic::Hash, int32_t>::value, "");
static_assert(basic::IsAvalanchingHasher<basic::Hash, uint64_t>::value, "");
static_assert(basic::IsAvalanchingHasher<basic::Hash, int64_t>::value, "");
static_assert(
    basic::IsAvalanchingHasher<basic::Hash, base::StringPiece>::value, "");
static_assert(basic::IsAvalanchingHasher<basic::Hash, std::string>::value, "");
static_assert(
    k32Bit == basic::IsAvalanchingHasher<basic::Hash, TestEnum>::value, "");
static_assert(basic::IsAvalanchingHasher<basic::Hash, TestBigEnum>::value, "");

static_assert(
    k32Bit ==
        basic::IsAvalanchingHasher<basic::hasher<uint8_t>, uint8_t>::value,
    "");
static_assert(
    k32Bit == basic::IsAvalanchingHasher<basic::hasher<char>, char>::value, "");
static_assert(
    k32Bit ==
        basic::IsAvalanchingHasher<basic::hasher<uint16_t>, uint16_t>::value,
    "");
static_assert(
    k32Bit ==
        basic::IsAvalanchingHasher<basic::hasher<int16_t>, int16_t>::value,
    "");
static_assert(
    k32Bit ==
        basic::IsAvalanchingHasher<basic::hasher<uint32_t>, uint32_t>::value,
    "");
static_assert(
    k32Bit ==
        basic::IsAvalanchingHasher<basic::hasher<int32_t>, int32_t>::value,
    "");
static_assert(
    basic::IsAvalanchingHasher<basic::hasher<uint64_t>, uint64_t>::value, "");
static_assert(
    basic::IsAvalanchingHasher<basic::hasher<int64_t>, int64_t>::value, "");
static_assert(
    basic::IsAvalanchingHasher<basic::hasher<float>, float>::value, "");
static_assert(
    basic::IsAvalanchingHasher<basic::hasher<double>, double>::value, "");
static_assert(
    basic::IsAvalanchingHasher<basic::hasher<std::string>, std::string>::value,
    "");
 static_assert(
     basic::IsAvalanchingHasher<basic::hasher<base::StringPiece>, std::string>::
         value,
     "");
 static_assert(
     basic::IsAvalanchingHasher<
         basic::hasher<base::StringPiece>,
         base::StringPiece>::value,
     "");
 static_assert(
     basic::IsAvalanchingHasher<
         basic::transparent<basic::hasher<base::StringPiece>>,
         base::StringPiece>::value,
     "");
...
static_assert(
    basic::IsAvalanchingHasher<
        basic::hasher<std::pair<int, int>>,
        std::pair<int, int>>::value,
    "");
static_assert(
    k32Bit ==
        basic::IsAvalanchingHasher<
            basic::hasher<std::tuple<int>>,
            std::tuple<int>>::value,
    "");
static_assert(
    basic::IsAvalanchingHasher<
        basic::hasher<std::tuple<std::string>>,
        std::tuple<std::string>>::value,
    "");
static_assert(
    basic::IsAvalanchingHasher<
        basic::hasher<std::tuple<int, int>>,
        std::tuple<int, int>>::value,
    "");
static_assert(
    basic::IsAvalanchingHasher<
        basic::hasher<std::tuple<int, int, int>>,
        std::tuple<int, int, int>>::value,
    "");
static_assert(
    k32Bit ==
        basic::IsAvalanchingHasher<basic::hasher<TestEnum>, TestEnum>::value,
    "");
static_assert(
    basic::IsAvalanchingHasher<basic::hasher<TestBigEnum>, TestBigEnum>::value,
    "");
...
TEST(Traits, basicHashUint64Avalances) {
  verifyAvalanching<basic::Hash>(uint64_t{0}, [](uint64_t& v) { v++; });
}

TEST(Traits, basicHasherInt64Avalances) {
  verifyAvalanching<basic::hasher<int64_t>>(
      int64_t{0}, [](int64_t& v) { v++; });
}

TEST(Traits, basicHasherFloatAvalanches) {
  verifyAvalanching<basic::hasher<float>>(0.0f, [](float& v) { v += 1; });
}

TEST(Traits, basicHasherDoubleAvalanches) {
  verifyAvalanching<basic::hasher<double>>(0.0, [](double& v) { v += 1; });
}
