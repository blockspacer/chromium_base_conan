...
#include <basic/synchronization/atomic_utils.h>
#include <basic/test/semaphore.h>
...
class DSchedThreadWrapper : public base::PlatformThread::Delegate {
 public:
  DSchedThreadWrapper(base::OnceClosure to_run);
  ~DSchedThreadWrapper() override;

  DSchedThreadWrapper(
      DSchedThreadWrapper&& other) {
    *this = std::move(other);
  }

  DSchedThreadWrapper& operator=(
      DSchedThreadWrapper&& other) {
    std::swap(thread_id_, other.thread_id_);
    std::swap(to_run_, other.to_run_);
    return *this;
  }
  ...
  base::PlatformThreadId GetThreadId() const {
    DCHECK_NE(thread_id_, base::kInvalidThreadId);
    return thread_id_;
  }

  void WaitUntilThreadIdCreated() {
    event.Wait();
    DCHECK_NE(thread_id_, base::kInvalidThreadId);
  }
 ...
 private:
  ...
  base::OnceClosure to_run_;

  base::PlatformThreadId thread_id_;

  DISALLOW_COPY_AND_ASSIGN(DSchedThreadWrapper);
};

// This is ugly, but better perf for DeterministicAtomic translates
// directly to more states explored and tested
#define BASIC_TEST_DSCHED_VLOG(...)                             \
  do {                                                          \
    if (false) {                                                \
      VLOG(2) << std::hex << base::PlatformThread::CurrentId() << ": " \
              << __VA_ARGS__;                                   \
    }                                                           \
  } while (false)
...
struct DSchedThreadId {
  base::PlatformThreadId val;
  explicit constexpr DSchedThreadId() : val(0) {}
  explicit constexpr DSchedThreadId(base::PlatformThreadId v) : val(v) {}
...
class DeterministicSchedule {
 public:
  ...
  static inline std::unique_ptr<base::Thread> thread(Func&& func, Args&&... args) {
    ...
    auto child_ptr = std::make_unique<base::Thread>("");
    //base::WaitableEvent event(base::WaitableEvent::ResetPolicy::AUTOMATIC,
    //                          base::WaitableEvent::InitialState::NOT_SIGNALED);
    child_ptr->Start();
    child_ptr->WaitUntilThreadStarted();
    child_ptr->task_runner()->PostTask(FROM_HERE, base::BindLambdaForTesting(
        [&](Args... a) {
          if (sched) {
            sched->afterThreadCreate(sem);
            beforeSharedAccess();
            BASIC_TEST_DSCHED_VLOG("running");
            afterSharedAccess();
          }
          base::ScopedClosureRunner scoped_destroy(
            base::BindOnce([](DeterministicSchedule* schedule){
              if (schedule) {
                schedule->beforeThreadExit();
              }
            }, REFERENCED(sched))
          );
          func(a...);
    ...
    if (sched) {
      beforeSharedAccess();
      sched->active_.insert(child_ptr->GetThreadId());
      BASIC_TEST_DSCHED_VLOG("forked " << std::hex << child_ptr->GetThreadId());
      afterSharedAccess();
    }
    return child_ptr;
  }
   ...
   *  schedule as the current thread. */
  template <typename Func, typename... Args>
  static inline std::unique_ptr<basic::test::DSchedThreadWrapper> thread(Func&& func, Args&&... args) {
    // TODO: maybe future versions of gcc will allow forwarding to thread
    atomic_thread_fence(std::memory_order_seq_cst);
    DeterministicSchedule* sched = getCurrentSchedule();
    Sem* sem = sched ? sched->beforeThreadCreate() : nullptr;
    auto child_ptr = std::make_unique<basic::test::DSchedThreadWrapper>(
      base::BindLambdaForTesting(
        ...
        args...));
    child_ptr->WaitUntilThreadIdCreated();
    if (sched) {
      beforeSharedAccess();
      sched->active_.insert(child_ptr->GetThreadId());
      BASIC_TEST_DSCHED_VLOG("forked " << std::hex << child_ptr->GetThreadId());
  ...
  static void join(basic::test::DSchedThreadWrapper& child);
   ...
   */
  static void joinAll(std::vector<std::unique_ptr<basic::test::DSchedThreadWrapper>>& children);
 ...
 private:
  ...
  std::unordered_set<base::PlatformThreadId> active_;
  std::unordered_map<base::PlatformThreadId, Sem*> joins_;
  std::unordered_map<base::PlatformThreadId, Sem*> exitingSems_;
  ...
  void waitForBeforeThreadExit(basic::test::DSchedThreadWrapper& child);
...
struct DeterministicAtomicImpl {
  ...
  bool compare_exchange_strong(
    ...
    return compare_exchange_strong(
        v0, v1, mo, ::basic::detail::default_failure_memory_order(mo));
  }
  bool compare_exchange_strong(
      T& v0,
      T v1,
  ...
  /* implicit */ operator T() const noexcept {
    ...
    BASIC_TEST_DSCHED_VLOG(this << "() -> " << std::hex << rv);
    Schedule::afterSharedAccess(true);
    return rv;
  }
  ...
  T load(std::memory_order mo = std::memory_order_seq_cst) const noexcept {
    Schedule::beforeSharedAccess();
    T rv = data_.load(mo);
    BASIC_TEST_DSCHED_VLOG(this << ".load() -> " << std::hex << rv);
    Schedule::afterSharedAccess(true);
    return rv;
  }
  ...
  T operator=(T v) noexcept {
    Schedule::beforeSharedAccess();
    T rv = (data_ = v);
    BASIC_TEST_DSCHED_VLOG(this << " = " << std::hex << v);
    Schedule::afterSharedAccess(true);
    return rv;
  }
  ...
  void store(T v, std::memory_order mo = std::memory_order_seq_cst) noexcept {
    Schedule::beforeSharedAccess();
    data_.store(v, mo);
    BASIC_TEST_DSCHED_VLOG(this << ".store(" << std::hex << v << ")");
    Schedule::afterSharedAccess(true);
  }
  ...
  T operator++() noexcept {
    Schedule::beforeSharedAccess();
    T rv = ++data_;
    BASIC_TEST_DSCHED_VLOG(this << " pre++ -> " << std::hex << rv);
    Schedule::afterSharedAccess(true);
    return rv;
  }
  ...
  T operator++(int /* postDummy */) noexcept {
    Schedule::beforeSharedAccess();
    T rv = data_++;
    BASIC_TEST_DSCHED_VLOG(this << " post++ -> " << std::hex << rv);
    Schedule::afterSharedAccess(true);
    return rv;
  }
...
struct DeterministicMutex {
  ...
  void lock() {
    BASIC_TEST_DSCHED_VLOG(this << ".lock()");
    DeterministicSchedule::beforeSharedAccess();
    while (!m.try_lock()) {
  ...
  bool try_lock() {
    ...
    BASIC_TEST_DSCHED_VLOG(this << ".try_lock() -> " << rv);
    DeterministicSchedule::afterSharedAccess();
    return rv;
  }

  void unlock() {
    BASIC_TEST_DSCHED_VLOG(this << ".unlock()");
    DeterministicSchedule::beforeSharedAccess();
    m.unlock();
    if (DeterministicSchedule::isActive()) {
      syncVar_.release();
