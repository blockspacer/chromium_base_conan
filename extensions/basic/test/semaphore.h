...
class Semaphore {
 public:
  ...
  void post(PrePost pre_post) {
    std::unique_lock<std::mutex> l{m_};
    CHECK_NE(value_, -size_t(1)) << "semaphore overflow";
    pre_post();
    if (!waiting_) {
      ++value_;
    } else {
...
class PolicySemaphore {
 public:
  ...
  void wait(PreWait pre_wait, PostWait post_wait) {
    ...
    if (value_) {
      ...
      const bool protect_post_wait = !is_empty_callable(post_wait);
      Waiter w;
      waiters_.push_back(w);
      w.protect_waiter_post_wait = protect_post_wait;
      w.wake_waiter.wait(lock);
      base::ScopedClosureRunner guard(
        base::BindOnce([](const bool protect_post_wait, Waiter& w, std::unique_lock<std::mutex>& lock){
            if (protect_post_wait) {
              w.wake_poster->post();
              w.wake_waiter.wait(lock);
            }
        }, protect_post_wait, REFERENCED(w), REFERENCED(lock))
      );
  ...
  void post(PrePost pre_post) {
    ...
    if (waiters_.empty()) {
      ...
      base::Optional<Event> wake_poster;
 ...
 private:
  ...
  Waiter& pull() {
    ...
    CHECK(false) << "(invalid argument) wake-policy";
  }
