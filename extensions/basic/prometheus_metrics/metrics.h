...
//
// MetricsRetirementAgeMs
// -----------------
// The minimum number of milliseconds a metric
...
// operator to track the metric on a per-tablet basic.
//
//
// Types of metrics
// ------------------------------------------------------------
...
//                            "Threads Started",
//                            ::basic::MetricUnit::kThreads,
//                            "Total number of threads started on this server",
//                            ::basic::EXPOSE_AS_COUNTER);
//
//
// Metrics ownership
// ------------------------------------------------------------
...
//
//   METRIC_DEFINE_counter(server, ping_requests, "Ping Requests", ::basic::MetricUnit::kRequests,
...
//   METRIC_DEFINE_counter(my_entity, ping_requests, "Ping Requests", ::basic::MetricUnit::kRequests,
...
#define METRIC_DEFINE_entity(name)                               \
  ::basic::MetricEntityPrototype METRIC_ENTITY_##name(#name)
...
#define METRIC_DEFINE_counter_with_level(entity, name, label, unit, desc, level)   \
  ::basic::CounterPrototype BOOST_PP_CAT(METRIC_, name)(                        \
      ::basic::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity), \
...
#define METRIC_DEFINE_counter(entity, name, label, unit, desc)   \
  METRIC_DEFINE_counter_with_level(entity, name, label, unit, desc, ::basic::MetricLevel::kInfo)
...
#define METRIC_DEFINE_lag_with_level(entity, name, label, desc, level) \
  ::basic::MillisLagPrototype BOOST_PP_CAT(METRIC_, name)( \
      ::basic::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity), \
                                      BOOST_PP_STRINGIZE(name), \
                                      label, \
...
#define METRIC_DEFINE_lag(entity, name, label, desc) \
  METRIC_DEFINE_lag_with_level(entity, name, label, desc, ::basic::MetricLevel::kInfo)

#define METRIC_DEFINE_gauge(type, entity, name, label, unit, desc, level, ...) \
  ::basic::GaugePrototype<type> BOOST_PP_CAT(METRIC_, name)(         \
      ::basic::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity), \
                                      BOOST_PP_STRINGIZE(name), \
                                      label, \
                                      unit, \
...
#define METRIC_DEFINE_gauge_string(entity, name, label, unit, desc, ...) \
    METRIC_DEFINE_gauge(std::string, entity, name, label, unit, desc, \
        ::basic::MetricLevel::kInfo, ## __VA_ARGS__)
...
#define METRIC_DEFINE_gauge_int64(entity, name, label, unit, desc, ...) \
    METRIC_DEFINE_gauge(int64_t, entity, name, label, unit, desc, \
        ::basic::MetricLevel::kInfo, ## __VA_ARGS__)
...
#define METRIC_DEFINE_histogram_with_percentiles(                              \
  ...
  ::basic::HistogramPrototype BOOST_PP_CAT(METRIC_, name)(                        \
      ::basic::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity),              \
                                      BOOST_PP_STRINGIZE(name), label, unit,   \
                                      desc,                                    \
                                      ::basic::MetricLevel::kInfo),                 \
      max_val, num_sig_digits, ::basic::ExportPercentiles::kTrue)
...
#define METRIC_DEFINE_histogram(entity, name, label, unit, desc, max_val,      \
  ...
  ::basic::HistogramPrototype BOOST_PP_CAT(METRIC_, name)(                        \
      ::basic::MetricPrototype::CtorArgs(BOOST_PP_STRINGIZE(entity),              \
                                      ...
                                      desc,                                    \
                                      ::basic::MetricLevel::kInfo),                 \
      max_val, num_sig_digits, ::basic::ExportPercentiles::kFalse)

// The following macros act as forward declarations for entity types and metric prototypes.
#define METRIC_DECLARE_entity(name) \
  extern ::basic::MetricEntityPrototype METRIC_ENTITY_##name
#define METRIC_DECLARE_counter(name)                             \
  extern ::basic::CounterPrototype METRIC_##name
#define METRIC_DECLARE_lag(name) \
  extern ::basic::LagPrototype METRIC_##name
#if TODO
#define METRIC_DECLARE_gauge_string(name) \
  extern ::basic::GaugePrototype<std::string> METRIC_##name
#endif
#define METRIC_DECLARE_gauge_bool(name) \
  extern ::basic::GaugePrototype<bool> METRIC_##name
#define METRIC_DECLARE_gauge_int32(name) \
  extern ::basic::GaugePrototype<int32_t> METRIC_##name
#define METRIC_DECLARE_gauge_uint32(name) \
  extern ::basic::GaugePrototype<uint32_t> METRIC_##name
#define METRIC_DECLARE_gauge_int64(name) \
  extern ::basic::GaugePrototype<int64_t> METRIC_##name
#define METRIC_DECLARE_gauge_uint64(name) \
  extern ::basic::GaugePrototype<uint64_t> METRIC_##name
#define METRIC_DECLARE_gauge_double(name) \
  extern ::basic::GaugePrototype<double> METRIC_##name
#define METRIC_DECLARE_histogram(name) \
  extern ::basic::HistogramPrototype METRIC_##name

#if defined(__APPLE__)
#define METRIC_DEFINE_gauge_size(entity, name, label, unit, desc, ...) \
  ::basic::GaugePrototype<size_t> METRIC_##name(                    \
      ::basic::MetricPrototype::CtorArgs(#entity, #name, label, unit, desc, ## __VA_ARGS__))
#define METRIC_DECLARE_gauge_size(name) \
  extern ::basic::GaugePrototype<size_t> METRIC_##name
#else
...
struct MetricPrometheusOptions {
  ...
  std::string metricsNodeName = "DEFAULT_NODE_NAME";

  bool exposeMetricHistogramPercentiles = true;
};
...
class MetricEntity : public base::RefCountedThreadSafe<MetricEntity> {
 public:
...
#if TODO
  typedef std::function<void (const MetricJsonOptions& opts)>
  ...
  typedef std::unordered_map<std::string, std::string > LabelsMap;

  LabelsMap labels() const {
    basic::subtle::SpinLock::Guard l(lock_);
    return labels_;
  }
  void set_label(const std::string& label, const std::string& value) {
    basic::subtle::SpinLock::Guard l(lock_);
    labels_[label] = value;
  }
  bool erase_label(const std::string& label, const std::string& value) {
    basic::subtle::SpinLock::Guard l(lock_);
  ...
  /// \note Avoid.
  /// All measurements of a single metric must have the same label set.
...
#if TODO
  scoped_refptr<StringGauge> FindOrCreateStringGauge(const GaugePrototype<std::string>* proto,
  ...
  void RetireOldMetrics(int metricsRetirementAgeMs);
  ...
  size_t num_metrics() const {
    basic::subtle::SpinLock::Guard guard(lock_);
 ...
 private:
  ...
  mutable basic::subtle::SpinLock lock_;
...
class PrometheusWriter {
 public:
  ...
  basic::Status WriteSingleEntry(
    ...
    if (it != attr.end()) {
      ...
      RETURN_IF_NOT_OK(FlushSingleEntry(attr, name, value));
    }
    RETURN_OK();
  }

#if TODO
  /// \note Prometheus can store only numeric string in value.
  /// Can be used to store BigInt.
  basic::Status WriteSingleStringEntry(
      const MetricEntity::AttributeMap& attr, const std::string& name, const std::string& value) {
    auto it = attr.find("table_id");
    if (it != attr.end()) {
      // For tablet level metrics, we roll up on the table level.
      if (per_table_attributes_.find(it->second) == per_table_attributes_.end()) {
        // If it's the first time we see this table, create the aggregate structures.
        per_table_attributes_[it->second] = attr;
        per_table_string_values_[it->second][name] = value;
      } else {
        auto type_it = attr.find("metric_type");
        if (type_it != attr.end() && type_it->second == "cdc") {
          /// \todo implement aggregation function
          per_table_string_values_[it->second][name] = value;
        } else {
          per_table_string_values_[it->second][name] += value;
        }
      ...
      // For non-tablet level metrics, export them directly.
      RETURN_IF_NOT_OK(FlushSingleStringEntry(attr, name, value));
    }
    RETURN_OK();
  }
#endif

  basic::Status FlushAggregatedValues() {
    for (const auto& entry : per_table_values_) {
      ...
      for (const auto& metric_entry : entry.second) {
        RETURN_IF_NOT_OK(FlushSingleEntry(attrs, metric_entry.first, metric_entry.second));
      }
    }
#if TODO
    for (const auto& entry : per_table_string_values_) {
      const auto& attrs = per_table_attributes_[entry.first];
      for (const auto& metric_entry : entry.second) {
        RETURN_IF_NOT_OK(FlushSingleStringEntry(attrs, metric_entry.first, metric_entry.second));
      }
    }
#endif
    RETURN_OK();
 ...
 private:
  ...
  virtual basic::Status FlushSingleEntry(const MetricEntity::AttributeMap& attr,
...
#if TODO
  virtual basic::Status FlushSingleStringEntry(const MetricEntity::AttributeMap& attr,
...
#if TODO
  // Can be used to store BigInt.
  std::map<std::string, std::map<std::string, std::string>> per_table_string_values_;
#endif
  // Output stream
...
class Metric : public base::RefCountedThreadSafe<Metric> {
 public:
  ...
  virtual basic::Status WriteAsJson(
                             const MetricJsonOptions& opts,
                             int metricsRetirementAgeMs) const = 0;
#endif

  // Exposes metrics to Prometheus using a simple text-based exposition format.
  // See prometheus.io/docs/instrumenting/exposition_formats/#text-based-format
  virtual basic::Status WriteForPrometheus(
...
class MetricRegistry {
 public:
  ...
  void tablets_shutdown_insert(std::string id) {
    std::lock_guard<std::shared_mutex> l(tablets_shutdown_lock_);
 ...
 private:
  ...
  mutable std::shared_mutex tablets_shutdown_lock_;
...
class MetricPrototypeRegistry {
 public:
  ...
  // prototypes.
  void WriteAsJson(int metricsRetirementAgeMs) const;
#endif
  ...
  void WriteAsJsonAndExit(int metricsRetirementAgeMs) const;
#endif
 private:
  friend class base::NoDestructor<MetricPrototypeRegistry>;
...
#if TODO
  scoped_refptr<StringGauge > InstantiateStringGauge(
      const scoped_refptr<MetricEntity>& entity,
      const std::string& initial_value) const {
    return entity->FindOrCreateStringGauge(this, initial_value);
  }
#endif
...
class Gauge : public Metric {
 public:
  ...
  virtual basic::Status WriteAsJson(
                             const MetricJsonOptions& opts,
                             int metricsRetirementAgeMs) const override;
#endif
 protected:
#if TODO
...
/// \note Prometheus can store only numeric string in value,
/// but can store arbitrary string in label
class StringGauge : public Gauge {
 public:
  ...
  // Exposes metrics to Prometheus using a simple text-based exposition format.
  // See prometheus.io/docs/instrumenting/exposition_formats/#text-based-format
  basic::Status WriteForPrometheus(
      PrometheusWriter* writer, MetricEntity::AttributeMap& attr,
      const MetricPrometheusOptions& opts,
      int metricsRetirementAgeMs) const override;
 ...
 protected:
#if TODO
  virtual void WriteValue() const override;
#endif
 private:
  std::string value_;
  LabelsMap labels_;
  mutable basic::subtle::SpinLock lock_;  // Guards value_
...
class AtomicGauge : public Gauge {
 public:
  ...
  virtual void set_value(const T& inValue) {
    T diffVal = inValue - value();
    /// \note `diffVal < 0` can decrement value.
    value_.IncrementBy(diffVal);
  }
  virtual LabelsMap labels() const {
    basic::subtle::SpinLock::Guard l(lock_);
    return labels_;
  }
  virtual void set_label(const std::string& label, const std::string& value) {
    basic::subtle::SpinLock::Guard l(lock_);
    labels_[label] = value;
  }
  virtual bool erase_label(const std::string& label, const std::string& value) {
    basic::subtle::SpinLock::Guard l(lock_);
    auto it = labels_.find(label);
    if (it == labels_.end()) return false;
    labels_.erase(it);
  ...
  /// \note Avoid.
  /// All measurements of a single metric must have the same label set.
  virtual void clear_all_labels() {
    labels_.clear();
  }
  void Increment() {
    value_.IncrementBy(1);
  }
...
// i.e. calls provided function on each `value()` call.
//
...
STRONGLY_TYPED_BOOL(ExportPercentiles);
...
class Histogram : public Metric {
 public:
  ...
  basic::Status GetAndResetHistogramSnapshotPB(HistogramSnapshotPB* snapshot,
                                const MetricJsonOptions& opts) const;
#endif


  // Returns a pointer to the underlying histogram.
  // The implementation of HdrHistogram is thread safe.
  const HdrHistogram* histogram() const { return histogram_.get(); }

  const MetricUnit::Type unit() const { return unit_; }
 ...
 private:
  ...
  FRIEND_TEST(MetricsTest, TestPrometheusWriter);
...
//
// // same as
// // basic::ScopedLatencyMetric scoped_processing_duration_hist_metric(
// //   processing_duration_hist_, basic::Auto::kTrue);
//
// SCOPED_LATENCY_METRIC(processing_duration_hist_, basic::Auto::kTrue);
//
#define SCOPED_LATENCY_METRIC(_h, ...) \
  basic::ScopedLatencyMetric scoped##_h##_metric(_h, ## __VA_ARGS__)
...
inline scoped_refptr<Counter> MetricEntity::FindOrCreateCounter(
  ...
  basic::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<Counter> m = basic::polymorphic_downcast<Counter*>(FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
    m = new Counter(proto);
...
inline scoped_refptr<MillisLag> MetricEntity::FindOrCreateMillisLag(
  ...
  CheckInstantiation(proto);
  basic::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<MillisLag> m = basic::polymorphic_downcast<MillisLag*>(FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
...
inline scoped_refptr<AtomicMillisLag> MetricEntity::FindOrCreateAtomicMillisLag(
  ...
  CheckInstantiation(proto);
  basic::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<AtomicMillisLag> m = basic::polymorphic_downcast<AtomicMillisLag*>(
...
inline scoped_refptr<Histogram> MetricEntity::FindOrCreateHistogram(
  ...
  CheckInstantiation(proto);
  basic::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<Histogram> m = basic::polymorphic_downcast<Histogram*>(FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
...
inline scoped_refptr<Histogram> MetricEntity::FindOrCreateHistogram(
  ...
  basic::subtle::SpinLock::Guard l(lock_);
  auto m = basic::polymorphic_downcast<Histogram*>(FindPtrOrNull(metric_map_, proto.get()).get());
  if (!m) {
...
inline scoped_refptr<AtomicGauge<T> > MetricEntity::FindOrCreateGauge(
  ...
  CheckInstantiation(proto);
  basic::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<AtomicGauge<T> > m = basic::polymorphic_downcast<AtomicGauge<T>*>(
      FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
...
inline scoped_refptr<AtomicGauge<T> > MetricEntity::FindOrCreateGauge(
  ...
  CheckInstantiation(proto.get());
  basic::subtle::SpinLock::Guard l(lock_);
  auto m = basic::polymorphic_downcast<AtomicGauge<T>*>(FindPtrOrNull(metric_map_, proto.get()).get());
  if (!m) {
...
inline scoped_refptr<FunctionGauge<T> > MetricEntity::FindOrCreateFunctionGauge(
  ...
  CheckInstantiation(proto);
  basic::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<FunctionGauge<T> > m = basic::polymorphic_downcast<FunctionGauge<T>*>(
      FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
...
#if TODO
inline scoped_refptr<StringGauge > MetricEntity::FindOrCreateStringGauge(
    const GaugePrototype<std::string>* proto,
    const std::string& initial_value) {
  CheckInstantiation(proto);
  basic::subtle::SpinLock::Guard l(lock_);
  scoped_refptr<StringGauge > m = basic::polymorphic_downcast<StringGauge*>(
      FindPtrOrNull(metric_map_, proto).get());
  if (!m) {
    m = new StringGauge(proto, initial_value);
    InsertOrDie(&metric_map_, proto, m);
  }
  return m;
}
