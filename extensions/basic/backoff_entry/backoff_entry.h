...
/// \note Retrying requests by default is not a good idea.
/// Retrying clients will increase the load on the server,
/// potentially causing an overload in what would otherwise
/// be a regular number of requests,
/// causing even more clients to issue retries (snowball effect).
///
/// For example, exponential backoff means varying the amount of time
/// between retries instead of having a fixed time window
/// (wait time is increased exponentially after every attempt).
///
/// If all the failed calls back off at the same time,
/// they increase overload even more. The solution is jitter.
...
class BASE_EXPORT BackoffEntry {
 public:
