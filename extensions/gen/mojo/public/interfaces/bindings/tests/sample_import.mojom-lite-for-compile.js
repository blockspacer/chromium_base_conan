// mojo/public/interfaces/bindings/tests/sample_import.mojom-lite-for-compile.js is auto generated by mojom_bindings_generator.py, do not edit

// Copyright 2018 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
'use strict';

goog.require('mojo.internal');
goog.require('mojo.internal.interfaceSupport');





goog.provide('imported.Shape');
goog.provide('imported.ShapeSpec');
/**
 * @const { {$: !mojo.internal.MojomType} }
 * @export
 */
imported.ShapeSpec = { $: mojo.internal.Enum() };

/**
 * @enum {number}
 * @export
 */
imported.Shape = {
  
  RECTANGLE: 1,
  CIRCLE: 2,
  TRIANGLE: 3,
  LAST: 3,
  MIN_VALUE: 1,
  MAX_VALUE: 3,
};

goog.provide('imported.AnotherShape');
goog.provide('imported.AnotherShapeSpec');
/**
 * @const { {$: !mojo.internal.MojomType} }
 * @export
 */
imported.AnotherShapeSpec = { $: mojo.internal.Enum() };

/**
 * @enum {number}
 * @export
 */
imported.AnotherShape = {
  
  RECTANGLE: 10,
  CIRCLE: 11,
  TRIANGLE: 12,
  MIN_VALUE: 10,
  MAX_VALUE: 12,
};

goog.provide('imported.YetAnotherShape');
goog.provide('imported.YetAnotherShapeSpec');
/**
 * @const { {$: !mojo.internal.MojomType} }
 * @export
 */
imported.YetAnotherShapeSpec = { $: mojo.internal.Enum() };

/**
 * @enum {number}
 * @export
 */
imported.YetAnotherShape = {
  
  RECTANGLE: 20,
  CIRCLE: 21,
  TRIANGLE: 22,
  MIN_VALUE: 20,
  MAX_VALUE: 22,
};



goog.provide('imported.ImportedInterface');
goog.provide('imported.ImportedInterfaceReceiver');
goog.provide('imported.ImportedInterfaceCallbackRouter');
goog.provide('imported.ImportedInterfaceInterface');
goog.provide('imported.ImportedInterfaceRemote');
goog.provide('imported.ImportedInterfacePendingReceiver');


/**
 * @implements {mojo.internal.interfaceSupport.PendingReceiver}
 * @export
 */
imported.ImportedInterfacePendingReceiver = class {
  /**
   * @param {!MojoHandle|!mojo.internal.interfaceSupport.Endpoint} handle
   */
  constructor(handle) {
    /** @public {!mojo.internal.interfaceSupport.Endpoint} */
    this.handle = mojo.internal.interfaceSupport.getEndpointForReceiver(handle);
  }

  /** @param {string=} scope */
  bindInBrowser(scope = 'context') {
    mojo.internal.interfaceSupport.bind(
        this.handle,
        imported.ImportedInterface.$interfaceName,
        scope);
  }
};

/** @interface */
imported.ImportedInterfaceInterface = class {
  
  /**
   */

  doSomething() {}
};

/**
 * @export
 * @implements { imported.ImportedInterfaceInterface }
 */
imported.ImportedInterfaceRemote = class {
  /** @param {MojoHandle|mojo.internal.interfaceSupport.Endpoint=} handle */
  constructor(handle = undefined) {
    /**
     * @private {!mojo.internal.interfaceSupport.InterfaceRemoteBase<!imported.ImportedInterfacePendingReceiver>}
     */
    this.proxy =
        new mojo.internal.interfaceSupport.InterfaceRemoteBase(
          imported.ImportedInterfacePendingReceiver,
          handle);

    /**
     * @public {!mojo.internal.interfaceSupport.InterfaceRemoteBaseWrapper<!imported.ImportedInterfacePendingReceiver>}
     */
    this.$ = new mojo.internal.interfaceSupport.InterfaceRemoteBaseWrapper(this.proxy);

    /** @public {!mojo.internal.interfaceSupport.ConnectionErrorEventRouter} */
    this.onConnectionError = this.proxy.getConnectionErrorEventRouter();
  }

  
  /**
   */

  doSomething() {
    this.proxy.sendMessage(
        912963854,
        imported.ImportedInterface_DoSomething_ParamsSpec.$,
        null,
        [
        ]);
  }
};

/**
 * An object which receives request messages for the ImportedInterface
 * mojom interface. Must be constructed over an object which implements that
 * interface.
 *
 * @export
 */
imported.ImportedInterfaceReceiver = class {
  /**
   * @param {!imported.ImportedInterfaceInterface } impl
   */
  constructor(impl) {
    /** @private {!mojo.internal.interfaceSupport.InterfaceReceiverHelperInternal<!imported.ImportedInterfaceRemote>} */
    this.helper_internal_ = new mojo.internal.interfaceSupport.InterfaceReceiverHelperInternal(
        imported.ImportedInterfaceRemote);

    /**
     * @public {!mojo.internal.interfaceSupport.InterfaceReceiverHelper<!imported.ImportedInterfaceRemote>}
     */
    this.$ = new mojo.internal.interfaceSupport.InterfaceReceiverHelper(this.helper_internal_);


    this.helper_internal_.registerHandler(
        912963854,
        imported.ImportedInterface_DoSomething_ParamsSpec.$,
        null,
        impl.doSomething.bind(impl));
    /** @public {!mojo.internal.interfaceSupport.ConnectionErrorEventRouter} */
    this.onConnectionError = this.helper_internal_.getConnectionErrorEventRouter();
  }
};

/**
 *  @export
 */
imported.ImportedInterface = class {
  /**
   * @return {!string}
   */
  static get $interfaceName() {
    return "imported.ImportedInterface";
  }

  /**
   * Returns a remote for this interface which sends messages to the browser.
   * The browser must have an interface request binder registered for this
   * interface and accessible to the calling document's frame.
   *
   * @return {!imported.ImportedInterfaceRemote}
   * @export
   */
  static getRemote() {
    let remote = new imported.ImportedInterfaceRemote;
    remote.$.bindNewPipeAndPassReceiver().bindInBrowser();
    return remote;
  }
};


/**
 * An object which receives request messages for the ImportedInterface
 * mojom interface and dispatches them as callbacks. One callback receiver exists
 * on this object for each message defined in the mojom interface, and each
 * receiver can have any number of listeners added to it.
 *
 * @export
 */
imported.ImportedInterfaceCallbackRouter = class {
  constructor() {
    this.helper_internal_ = new mojo.internal.interfaceSupport.InterfaceReceiverHelperInternal(
      imported.ImportedInterfaceRemote);

    /**
     * @public {!mojo.internal.interfaceSupport.InterfaceReceiverHelper<!imported.ImportedInterfaceRemote>}
     */
    this.$ = new mojo.internal.interfaceSupport.InterfaceReceiverHelper(this.helper_internal_);

    this.router_ = new mojo.internal.interfaceSupport.CallbackRouter;

    /**
     * @public {!mojo.internal.interfaceSupport.InterfaceCallbackReceiver}
     */
    this.doSomething =
        new mojo.internal.interfaceSupport.InterfaceCallbackReceiver(
            this.router_);

    this.helper_internal_.registerHandler(
        912963854,
        imported.ImportedInterface_DoSomething_ParamsSpec.$,
        null,
        this.doSomething.createReceiverHandler(false /* expectsResponse */));
    /** @public {!mojo.internal.interfaceSupport.ConnectionErrorEventRouter} */
    this.onConnectionError = this.helper_internal_.getConnectionErrorEventRouter();
  }

  /**
   * @param {number} id An ID returned by a prior call to addListener.
   * @return {boolean} True iff the identified listener was found and removed.
   * @export
   */
  removeListener(id) {
    return this.router_.removeListener(id);
  }
};


goog.provide('imported.PointSpec');
/**
 * @const { {$:!mojo.internal.MojomType}}
 * @export
 */
imported.PointSpec =
    { $: /** @type {!mojo.internal.MojomType} */ ({}) };

goog.provide('imported.ImportedInterface_DoSomething_ParamsSpec');
/**
 * @const { {$:!mojo.internal.MojomType}}
 * @export
 */
imported.ImportedInterface_DoSomething_ParamsSpec =
    { $: /** @type {!mojo.internal.MojomType} */ ({}) };

goog.provide('imported.PointOrShapeSpec');
/**
 * @const { {$:!mojo.internal.MojomType} }
 * @export
 */
imported.PointOrShapeSpec =
    { $: /** @type {!mojo.internal.MojomType} */ ({}) };




mojo.internal.Struct(
    imported.PointSpec.$,
    'Point',
    [
      mojo.internal.StructField(
        'x', 0,
        0,
        mojo.internal.Int32,
        0,
        false /* nullable */),
      mojo.internal.StructField(
        'y', 4,
        0,
        mojo.internal.Int32,
        0,
        false /* nullable */),
    ],
    [[0, 16],]);



goog.provide('imported.Point');

/** @record */
imported.Point = class {
  constructor() {
    /** @export { !number } */
    this.x;
    /** @export { !number } */
    this.y;
  }
};




mojo.internal.Struct(
    imported.ImportedInterface_DoSomething_ParamsSpec.$,
    'ImportedInterface_DoSomething_Params',
    [
    ],
    [[0, 8],]);



goog.provide('imported.ImportedInterface_DoSomething_Params');

/** @record */
imported.ImportedInterface_DoSomething_Params = class {
  constructor() {
  }
};


goog.provide('imported.PointOrShape');


mojo.internal.Union(
    imported.PointOrShapeSpec.$, 'PointOrShape',
    {
      'point': {
        'ordinal': 0,
        'type': imported.PointSpec.$,
      },
      'shape': {
        'ordinal': 1,
        'type': imported.ShapeSpec.$,
      },
    });

/**
 * @typedef { {
 *   point: (!imported.Point|undefined),
 *   shape: (!imported.Shape|undefined),
 * } }
 */
imported.PointOrShape;
