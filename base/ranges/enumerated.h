...
template <typename T,
          typename TIter = decltype(ranges::begin(std::declval<T>())),
          typename = decltype(ranges::end(std::declval<T>()))>
constexpr auto enumerated(T && iterable)
{
  ...
  {
    T iterable;
    auto begin() { return iterator{ 0, ranges::begin(iterable) }; }
    auto end() { return iterator{ 0, ranges::end(iterable) }; }
  };
