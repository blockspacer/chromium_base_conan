...
#include "base/hash/murmurhash.h"
...
static size_t GetHash(const base::span<const uint8_t>& slice) {
  ...
  static basis::murmur_hash hash;
  auto result = hash(slice);
  uint64_t result2 = basis::MurmurHash2_64(slice.data(), static_cast<int>(slice.size()), hash_seed);
...
static size_t GetHashWithSeed(const base::span<const uint8_t>& slice) {
  ...
  auto result = basis::MurmurHash(slice.data(), static_cast<int>(slice.size()), hash_seed);
  uint64_t result2 = basis::MurmurHash2_64(slice.data(), static_cast<int>(slice.size()), hash_seed);
  EXPECT_EQ(result, result2);
  return result;
}
